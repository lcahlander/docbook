...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="h f m fn db t" version="2.0">
    <xsl:template match="db:index|db:setindex">
        <article>
            ...
            <xsl:apply-templates select="*[not(self::db:indexdiv)                                    and not(self::db:indexentry)]"/>
...
<!-- ==================================================================== -->
    ...
    <xsl:template match="db:primaryie">
        ...
        <xsl:choose>
            ...
            <xsl:when test="following-sibling::db:seeie                     |following-sibling::db:seealsoie">
                <dd>
                    <dl>
                        <xsl:apply-templates select="following-sibling::db:seeie                                        |following-sibling::db:seealsoie"/>
                    </dl>
                </dd>
            </xsl:when>
        </xsl:choose>
    ...
    <xsl:template match="db:tertiaryie">
        ...
        <xsl:if test="following-sibling::db:seeie                 |following-sibling::db:seealsoie">
            <dd>
                <dl>
                    <xsl:apply-templates select="following-sibling::db:seeie                                      |following-sibling::db:seealsoie"/>
                </dl>
