...
-->
    <xsl:template match="db:glossary[@role='auto']">
        ...
        <xsl:variable name="glossary">
            <db:glossary>
                ...
                <xsl:variable name="divs" select="db:glossdiv"/>
                <xsl:choose>
    ...
    <xsl:template match="db:glossentry/db:acronym">
        ...
        <xsl:if test="following-sibling::db:acronym   |following-sibling::db:abbrev">, </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="db:glossseealso">
        <xsl:variable name="target" select="if (key('id', @otherterm))    then key('id', @otherterm)[1]    else key('glossterm', string(.))"/>
        <xsl:choose>
            <xsl:when test="$target">
                <a href="{f:href(/,$target)}">
                    <xsl:apply-templates select="$target" mode="m:xref-to"/>
