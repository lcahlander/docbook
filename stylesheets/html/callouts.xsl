...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="db doc f ghost h m t u xlink xs" version="2.0">
  ...
  -->
        <xsl:call-template name="t:callout-bug">
            <xsl:with-param name="conum">
                <xsl:number count="db:co" level="any" format="1"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    <xsl:template name="t:callout-bug">
        <xsl:param name="conum" select="1"/>
        <xsl:variable name="iconum" select="if ($conum castable as xs:decimal)                         then xs:decimal($conum) else 1"/>
        <xsl:choose>
            <xsl:when test="$callout.graphics != 0                     and $iconum &lt;= number($callout.graphics.number.limit)">
                <img src="{$callout.graphics.path}{$iconum}{$callout.graphics.extension}" alt="{$iconum}" border="0"/>
            </xsl:when>
            <xsl:when test="$callout.unicode != 0                     and $iconum &lt;= number($callout.unicode.number.limit)">
                <xsl:choose>
    ...
    <xsl:template match="db:calloutlist">
        <div>
            ...
            <xsl:apply-templates select="*[not(self::db:info)        and not(self::db:callout)]"/>
                    ...
                    <xsl:for-each select="tokenize(@arearefs,'\s')">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="$target/self::db:areaset">
                                <xsl:call-template name="t:callout-bug">
                                    <xsl:with-param name="conum" select="count($target/preceding-sibling::db:areaset      |$target/preceding-sibling::db:area)      +1"/>
                                </xsl:call-template>
                            </xsl:when>
                            <xsl:when test="$target/self::db:area">
                                <xsl:choose>
                                    <xsl:when test="$target/parent::db:areaset">
                                        <xsl:call-template name="t:callout-bug">
                                            <xsl:with-param name="conum" select="count($target/parent::db:areaset/preceding-sibling::db:areaset          |$target/parent::db:areaset/preceding-sibling::db:area)          +1"/>
                                        </xsl:call-template>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:call-template name="t:callout-bug">
