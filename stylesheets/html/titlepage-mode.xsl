...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:t="http://docbook.org/xslt/ns/template" version="2.0" exclude-result-prefixes="db f m t xs h">
...
<!-- Titlepage templates (also used for recto) -->
  ...
  <!-- generated text. We *don't* have to call m:object-title-markup here -->
        <xsl:choose>
            <xsl:when test="$context/self::db:bibliography                     or $context/self::db:preface                     or $context/self::db:dedication                     or $context/self::db:colophon                     or $context/self::db:dedication                     or $context/self::db:glossary                     or $context/self::db:setindex                     or $context/self::db:index">
                <xsl:choose>
            ...
            <xsl:when test="$context/self::db:warning                     or $context/self::db:caution                     or $context/self::db:tip                     or $context/self::db:note                     or $context/self::db:important">
                <h3>
                    <xsl:apply-templates/>
                </h3>
            </xsl:when>
    ...
    <xsl:template match="db:set/db:title|db:set/db:info/db:title                      |db:book/db:title|db:book/db:info/db:title                      |db:part/db:title|db:part/db:info/db:title                      |db:reference/db:title|db:reference/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h1>
    ...
    <xsl:template match="db:preface/db:title|db:prefac/db:info/db:title                      |db:chapter/db:title|db:chapter/db:info/db:title                      |db:appendix/db:title|db:appendix/db:info/db:title                      |db:dedication/db:title|db:dedication/db:info/db:title                      |db:colophon/db:title|db:colophon/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h2>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:bibliography/db:title|db:bibliography/db:info/db:title                      |db:glossary/db:title|db:glossary/db:info/db:title                      |db:index/db:title|db:index/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <xsl:choose>
    ...
    <xsl:template match="db:bibliodiv/db:title|db:bibliodiv/db:info/db:title                      |db:glossdiv/db:title|db:glossdiv/db:info/db:title                      |db:indexdiv/db:title|db:indexdiv/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <xsl:choose>
            <xsl:when test="ancestor::db:article">
                <h4>
    ...
    <xsl:template match="db:bibliolist/db:title|db:bibliolist/db:info/db:title                      |db:glosslist/db:title|db:glosslist/db:info/db:title                      |db:blockquote/db:title|db:blockquote/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
  ...
  <!-- lazy -->
    ...
    <xsl:template match="db:sect1/db:title|db:sect1/db:info/db:title                      |db:refsect1/db:title|db:refsect1/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h2>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:sect2/db:title|db:sect2/db:info/db:title                      |db:refsect2/db:title|db:refsect2/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h3>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:sect3/db:title|db:sect3/db:info/db:title                      |db:refsect3/db:title|db:refsect3/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h4>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:figure/db:title|db:figure/db:info/db:title                      |db:example/db:title|db:example/db:info/db:title                      |db:table/db:title|db:table/db:info/db:title                      |db:equation/db:title|db:equation/db:info/db:title                      |db:procedure/db:title|db:procedure/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <div class="title">
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:warning/db:title|db:warning/db:info/db:title                      |db:note/db:title|db:note/db:info/db:title                      |db:caution/db:title|db:caution/db:info/db:title                      |db:important/db:title|db:important/db:info/db:title                      |db:tip/db:title|db:tip/db:info/db:title                      |db:sidebar/db:title|db:sidebar/db:info/db:title                      |db:task/db:title|db:task/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h3>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:tasksummary/db:title|db:tasksummary/db:info/db:title                      |db:taskprerequisites/db:title|db:taskprerequisites/db:info/db:title                      |db:taskrelated/db:title|db:taskrelated/db:info/db:title                      |db:task/db:procedure/db:title|db:task/db:procedure/db:info/db:title" mode="m:titlepage-mode" priority="100">
  <!-- explicit priority to disambiguate db:procedure matching -->
    ...
    <xsl:template match="db:segmentedlist/db:title|db:segmentedlist/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <div class="title">
            <strong>
                <xsl:apply-templates select="$context" mode="m:object-title-markup">
    ...
    <xsl:template match="db:step/db:title|db:step/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h4>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:qandaset/db:title|db:qandaset/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h2>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
    ...
    <xsl:template match="db:qandadiv/db:title|db:qandadiv/db:info/db:title" mode="m:titlepage-mode">
        <xsl:variable name="context" select="if (parent::db:info) then parent::db:info/parent::* else parent::*"/>
        <h3>
            <xsl:apply-templates select="$context" mode="m:object-title-markup">
                <xsl:with-param name="allow-anchors" select="true()"/>
...
<!-- FIXME: othercredit deserves its own template, with gentext more like db:editor -->
    ...
    <xsl:template match="*" mode="m:titlepage-mode">
        <xsl:message>No m:titlepage-mode template for <xsl:value-of select="node-name(.)"/>
        </xsl:message>
        <xsl:apply-templates select="."/>
    </xsl:template>
