...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn h m t" version="2.0">
...
-->
    <xsl:template match="db:funcprototype">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="pis" select="ancestor::db:funcsynopsis        //processing-instruction('dbhtml')"/>
...
<!-- funcprototype: kr, tabular -->
    <xsl:template match="db:funcprototype" mode="m:kr-tabular">
        <table border="0" summary="Function synopsis" cellspacing="0" cellpadding="0" style="padding-bottom: 1em">
            ...
            <tr>
                ...
                <xsl:apply-templates select="(db:void|db:varargs|db:paramdef)[1]" mode="m:kr-tabular"/>
            </tr>
      ...
      -->
                <xsl:apply-templates select="db:paramdef" mode="m:kr-tabular-funcsynopsis-mode"/>
            </table>
        </xsl:if>
    </xsl:template>
        ...
        <tr>
            <xsl:choose>
                <xsl:when test="db:type and db:funcparams">
                    <td>
                        <xsl:apply-templates select="db:type" mode="m:kr-tabular-funcsynopsis-mode"/>
                        <xsl:text>&#160;</xsl:text>
                    </td>
                    <td>
                        <xsl:apply-templates select="db:type/following-sibling::node()" mode="m:kr-tabular-funcsynopsis-mode"/>
                    </td>
                </xsl:when>
                ...
                <xsl:otherwise>
                    <td>
                        <xsl:apply-templates select="db:parameter/preceding-sibling::node()[not(self::db:parameter)]" mode="m:kr-tabular-funcsynopsis-mode"/>
                        <xsl:text>&#160;</xsl:text>
                    </td>
                    <td>
                        <xsl:apply-templates select="db:parameter" mode="m:kr-tabular"/>
                        <xsl:apply-templates select="db:parameter/following-sibling::node()[not(self::db:parameter)]" mode="m:kr-tabular-funcsynopsis-mode"/>
                        <xsl:text>;</xsl:text>
                    </td>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template match="db:paramdef/db:parameter" mode="m:kr-tabular-funcsynopsis-mode">
        <xsl:choose>
            <xsl:when test="$funcsynopsis.decoration != 0">
                <var class="pdparam">
                    <xsl:apply-templates mode="m:kr-tabular-funcsynopsis-mode"/>
...
<!-- funcprototype: ansi, tabular -->
    ...
    -->
            <tr>
                ...
                <xsl:apply-templates select="(db:void|db:varargs|db:paramdef)[1]" mode="m:ansi-tabular"/>
            </tr>
            <xsl:for-each select="(db:void|db:varargs|db:paramdef)[position() &gt; 1]">
                <tr>
                    <td>&#160;</td>
    ...
    <xsl:template match="db:paramdef" mode="m:ansi-tabular">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <td>
                    <xsl:apply-templates select="db:parameter/preceding-sibling::node()[not(self::db:parameter)]" mode="m:ansi-tabular"/>
                    <xsl:text>&#160;</xsl:text>
                </td>
                <td>
                    <xsl:apply-templates select="db:parameter" mode="m:ansi-tabular"/>
                    <xsl:apply-templates select="db:parameter/following-sibling::node()[not(self::db:parameter)]" mode="m:ansi-tabular"/>
                    <xsl:choose>
                        <xsl:when test="following-sibling::*">
                            <xsl:text>, </xsl:text>
                        </xsl:when>
...
<!-- XSLT2 templates and functions -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:funcprototype-xslt2-function">
    ...
    <xsl:template match="db:funcprototype" mode="m:funcprototype-xslt2-function">
        <xsl:apply-templates select="db:funcdef/db:function" mode="m:funcprototype-xslt2-function"/>
        <xsl:text>(</xsl:text>
        <xsl:apply-templates select="db:paramdef" mode="m:funcprototype-xslt2-function"/>
        <xsl:text>)</xsl:text>
        <xsl:if test="db:funcdef/db:type">
            <xsl:text> as </xsl:text>
            <xsl:apply-templates select="db:funcdef/db:type" mode="m:funcprototype-xslt2-function"/>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="db:paramdef" mode="m:funcprototype-xslt2-function">
        <xsl:apply-templates select="db:parameter" mode="m:funcprototype-xslt2-function"/>
        <xsl:if test="db:type">
            <xsl:text> as </xsl:text>
            <xsl:apply-templates select="db:type" mode="m:funcprototype-xslt2-function"/>
        </xsl:if>
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:funcprototype-xslt2-template">
    ...
    <xsl:template match="db:funcprototype" mode="m:funcprototype-xslt2-template">
        <pre>
            ...
            <xsl:apply-templates select="db:paramdef" mode="m:funcprototype-xslt2-template"/>
