...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fp="http://docbook.org/xslt/ns/extension" exclude-result-prefixes="db doc f fp ghost h m mp t tp u xs" version="2.0">
  ...
  -->
        <xsl:variable name="pl-no-lb" as="node()*">
            <xsl:apply-templates select="$pl-empty-tags" mode="mp:pl-no-lb"/>
        </xsl:variable>
  ...
  -->
        <xsl:variable name="pl-removed-lines" as="node()*">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$everyNth &gt; 0                    and count($pl-lines) &gt;= $minlines">
                            <xsl:apply-templates select="$pl-callouts" mode="mp:pl-restore-lines">
...
<!-- ============================================================ -->
        ...
        <xsl:call-template name="t:callout-bug">
            <xsl:with-param name="conum">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:number count="ghost:co" level="any" from="db:programlisting|db:screen|db:literallayout|db:synopsis" format="1"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="mp:expand-ghost-co">
        <xsl:copy/>
    </xsl:template>
...
<!-- ============================================================ -->
    <xsl:template match="*" mode="mp:pl-cleanup">
        ...
        <xsl:element name="{local-name(.)}" namespace="{namespace-uri(.)}">
            <xsl:copy-of select="@*[name(.) != 'id'                             and namespace-uri(.) != 'http://docbook.org/ns/docbook/ephemeral']"/>
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="mp:pl-cleanup">
        <xsl:copy/>
    </xsl:template>
...
<!-- ============================================================ -->
    <xsl:function name="f:lastLineNumber" as="xs:decimal">
        ...
        <xsl:variable name="startnum" as="xs:decimal">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:variable name="prev" select="$listing/preceding::*[node-name(.)=node-name($listing)][1]"/>
                    <xsl:choose>
        ...
        <xsl:variable name="expanded-text" as="node()*">
            <xsl:for-each select="$listing/node()">
                <xsl:choose>
                    <xsl:when test="self::db:inlinemediaobject    and db:textobject/db:textdata">
    ...
    <xsl:template match="ghost:line" mode="mp:pl-restore-lines">
        ...
        <xsl:variable name="startnum" as="xs:decimal">
            <xsl:choose>
                <xsl:when test="not($element/@continuation)                       or $element/@continuation != 'continues'">
                    <xsl:value-of select="0"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:variable name="prev" select="$element/preceding::*[node-name(.)=node-name($element)][1]"/>
                    <xsl:choose>
                        <xsl:when test="empty($prev)">
                            <xsl:value-of select="0"/>
                        </xsl:when>
        ...
        <xsl:if test="$everyNth &gt; 0">
            <xsl:choose>
                <xsl:when test="$linenumber = 1         or $linenumber mod $everyNth = 0">
                    <xsl:variable name="numwidth" select="string-length(string($linenumber))"/>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:restore-content">
...
an element that has content.</para>
  ...
  <!-- FIXME: I totally do not understand why straight-up node comparisons fail here.
       ...
       with generate-id(). -->
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="follmarkids" select="for $node in ($mark, $mark/following-sibling::node()) return generate-id($node)"/>
	...
	<xsl:value-of select="count($end)"/>
      ...
      -->
                <xsl:variable name="endpos" select="f:find-node-in-sequence($rest, $end, 2)"/>
                <xsl:element name="{name($mark)}" namespace="{namespace-uri($mark)}">
                    <xsl:copy-of select="$mark/@*"/>
                    <xsl:call-template name="t:restore-content">
                        <xsl:with-param name="nodes" select="subsequence($rest, 2, $endpos - 2)"/>
                    </xsl:call-template>
                </xsl:element>
                <xsl:call-template name="t:restore-content">
                    <xsl:with-param name="nodes" select="subsequence($rest, $endpos+1)"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
            ...
            <xsl:when test="$nodes[1] instance of text()">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:call-template name="tp:addcallout">
                            <xsl:with-param name="area" select="$area"/>
                            <xsl:with-param name="nodes" select="($rest, $nodes[position() &gt; 1])"/>
                            <xsl:with-param name="pos" select="$pos+1"/>
                        </xsl:call-template>
                    </xsl:otherwise>
                </xsl:choose>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:unwrap">
...
<parameter>unwrap</parameter> are at the top level.</para>
    ...
    <xsl:template name="t:unwrap">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$content[1] instance of element()                     and node-name($content[1]) = $unwrap">
...
<!-- ============================================================ -->
    ...
    <xsl:template match="text()" as="node()*" mode="mp:pl-no-lb">
        <xsl:choose>
    <!-- There's a bug in MarkLogic server V4.2-1 that coalesces the nodes returned
         by xsl:analyze-string into a single node. Let's work around that. -->
            <xsl:when test="system-property('xsl:vendor') = 'MarkLogic Corporation'                     and system-property('xsl:product-version') = '4.2-1'">
    ...
    <!-- In later versions of 4.2 (starting I don't know when, but at least in 4.2-6)
         there's a different bug. Not fixed in 5.0-1 either. Or 6.x -->
            <xsl:when test="system-property('xsl:vendor') = 'MarkLogic Corporation'                     and (starts-with(system-property('xsl:product-version'), '4.2')                          or starts-with(system-property('xsl:product-version'), '5')                          or starts-with(system-property('xsl:product-version'), '6'))">
                <xsl:variable name="parts" as="item()*">
                    <xsl:analyze-string select="." regex="\n">
                        <xsl:matching-substring>
                            <ghost:br/>
    ...
    <xsl:template match="comment()|processing-instruction()" as="node()*" mode="mp:pl-no-lb">
        <xsl:copy/>
    </xsl:template>

<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:patch-empty-elements">
...
<function role="named-template">t:restore-content</function>.</para>
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="m:patch-empty-elements">
        <xsl:copy/>
    </xsl:template>

<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:remove-lb">
...
in a verbatim environment.</para>
    ...
    <xsl:template match="text()" mode="m:remove-lb">
        <xsl:value-of select="replace(.,'&#xA;',' ')"/>
    </xsl:template>
    <xsl:template match="comment()|processing-instruction()" mode="m:remove-lb">
        <xsl:copy/>
    </xsl:template>
...
<!-- ============================================================ -->
    ...
    <xsl:template match="db:areaspec/db:area">
        <db:area>
            ...
            <xsl:if test="(not(@units)      or @units='linecolumn'      or @units='linecolumnpair')">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:variable name="toks" select="tokenize(normalize-space(@coords), ' ')"/>
                        <xsl:attribute name="ghost:line" select="$toks[1]"/>
                        <xsl:attribute name="ghost:col" select="$toks[2]"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            <xsl:attribute name="ghost:number" select="count(preceding-sibling::db:area                                  |preceding-sibling::db:areaset)+1"/>
        </db:area>
    </xsl:template>
    <xsl:template match="db:areaset/db:area">
        <db:area>
            ...
            <xsl:attribute name="ghost:number" select="count(parent::db:areaset/preceding-sibling::db:area                              |parent::db:areaset/preceding-sibling::db:areaset)       + 1"/>
        </db:area>
    </xsl:template>
</xsl:stylesheet>