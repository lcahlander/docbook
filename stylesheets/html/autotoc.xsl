...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn h m mp t tp xs" version="2.0">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="mp:toc">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="tp:make-toc">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:make-lots">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:division-toc">
...
division (book, part, etc.).</para>
    ...
    <xsl:template name="t:division-toc">
        ...
        <xsl:call-template name="tp:make-toc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:part|db:reference        |db:preface|db:chapter|db:appendix        |db:article        |db:bibliography|db:glossary|db:index        |db:refentry        |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>

<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:component-toc">
...
component (chapter, article, etc.).</para>
    ...
    <xsl:template name="t:component-toc">
        ...
        <xsl:call-template name="tp:make-toc">
            ...
            <xsl:with-param name="nodes" select="db:section|db:sect1|db:refentry        |db:article|db:bibliography|db:glossary        |db:appendix|db:index        |db:bridgehead[not(@renderas)                       and $bridgehead.in.toc != 0]        |.//db:bridgehead[@renderas='sect1'                          and $bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>

<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:component-toc-separator">
...
section.</para>
    ...
    <xsl:template name="t:section-toc">
        ...
        <xsl:call-template name="tp:make-toc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="toc.title" select="$toc.title"/>
            <xsl:with-param name="nodes" select="db:section        |db:sect1|db:sect2|db:sect3|db:sect4|db:sect5        |db:refentry        |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>

<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:section-toc-separator">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:qanda-toc">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:qanda-toc-separator">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="tp:subtoc">
	...
	<!-- sigh... -->
        ...
        <xsl:variable name="show.subtoc" as="xs:boolean" select="count($subtoc/*) &gt; 0    and $toc.section.depth &gt; $depth and exists($nodes)    and $toc.max.depth &gt; $depth.from.context"/>
        <li>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="tp:toc-line">
  ...
  <!-- If the setindex tag is not empty, it should be it in the TOC -->
    ...
    <xsl:template match="db:part|db:reference" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:appendix|db:chapter|db:article                                          |db:index|db:glossary|db:bibliography                                          |db:preface|db:reference|db:refentry                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="db:preface|db:chapter|db:appendix|db:article" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:section|db:sect1|db:simplesect|db:refentry                                          |db:glossary|db:bibliography|db:index                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="db:sect1" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:sect2                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="db:sect2" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:sect3                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="db:sect3" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:sect4                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    <xsl:template match="db:sect4" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:sect5                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="db:section" mode="mp:toc">
        ...
        <xsl:call-template name="tp:subtoc">
            <xsl:with-param name="toc-context" select="$toc-context"/>
            <xsl:with-param name="nodes" select="db:section                                          |db:bridgehead[$bridgehead.in.toc != 0]"/>
        </xsl:call-template>
    </xsl:template>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:manual-toc">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="tp:list-of-titles">
    ...
    <xsl:template match="db:figure|db:table|db:example|db:equation|db:procedure" mode="mp:toc">
        <xsl:param name="toc-context" select="."/>
        <li>
            <xsl:variable name="label">
                <xsl:apply-templates select="." mode="m:label-content"/>
