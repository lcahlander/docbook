...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xdmp="http://marklogic.com/xdmp" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:ext="http://docbook.org/extensions/xslt20" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="doc h f m mp fn db t ext xdmp xs tp" version="2.0">
    ...
    <xsl:template match="db:programlistingco|db:screenco">
        ...
        <xsl:variable xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" name="areas" as="element()*">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:verbatim">
    ...
    <xsl:template match="db:literallayout|db:address" mode="m:verbatim">
                ...
                <xsl:when test="$asTable">
                    <xsl:variable name="startnum" as="xs:decimal">
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:variable name="prev" select="preceding::*[node-name(.)=node-name(.)][1]"/>
                                <xsl:choose>
                    ...
                    <xsl:variable name="wrapper" select="if ((self::db:literallayout and not(@class='monospaced'))                                    or self::db:address)                               then 'div'                               else 'pre'"/>
                            ...
                            <td valign="top" align="right" class="verbatimlinenumbers">
                                <xsl:element name="{$wrapper}">
                                    <xsl:for-each select="$lines">
                                        ...
                                        <div class="verbatimline verbatimlinenumber {                               if (position() mod 2 = 0) then 'verbatimeven' else 'verbatimodd'                               }">
                                            <div class="vlcontent">
    ...
    <xsl:template match="comment()|processing-instruction()" mode="mp:literallayout">
        <xsl:copy/>
    </xsl:template>
...
<!-- This is a highlight implementation that works on MarkLogic server.
    ...
    <xsl:function use-when="function-available('xdmp:http-post')" name="ext:highlight" as="node()*">
        <xsl:param name="code"/>
        <xsl:param name="language"/>
        <xsl:variable name="code-node" as="text()">
            <xsl:value-of select="$code"/>
        </xsl:variable>
        <xsl:variable name="highlighted" select="xdmp:http-post(concat($pygmenter-uri,'?language=',$language),(),$code-node)"/>
