...
<!-- ============================================================ -->
    <xsl:template match="db:orderedlist">
        <xsl:variable name="starting.number" select="f:orderedlist-starting-number(.)"/>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="simplelist-horiz">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="simplelist-horiz-row">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="simplelist-vert">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="simplelist-vert-row">
        ...
        <xsl:call-template name="t:semiformal-object">
            ...
            <xsl:with-param name="object" as="element()">
                <div>
                    ...
                    <xsl:apply-templates select="(db:step[1]/preceding-sibling::node())[not(self::db:info)]"/>
                    <xsl:choose>
                        <xsl:when test="count(db:step) = 1">
                            <ul>
                                <xsl:apply-templates select="db:step[1]|db:step[1]/following-sibling::node()"/>
                            </ul>
                        </xsl:when>
                        <xsl:otherwise>
                            <ol>
...
<!-- ==================================================================== -->
    <xsl:template match="db:segmentedlist">
        <xsl:variable name="presentation" select="f:pi(processing-instruction('dbhtml'),         'list-presentation')"/>
        <div>
            <xsl:sequence select="f:html-attributes(.)"/>
            <xsl:call-template name="t:titlepage"/>
            <xsl:choose>
  ...
  -->
        <div>
            ...
            <strong>
                <span class="segtitle">
                    <xsl:apply-templates select="$segtitles[$segnum=position()]" mode="m:segtitle-in-seg"/>
                    <xsl:text>: </xsl:text>
                </span>
            </strong>
            <xsl:apply-templates/>
    ...
    <xsl:template match="db:segmentedlist" mode="m:seglist-table">
        <xsl:variable name="table-summary" select="f:pi(processing-instruction('dbhtml'),         'table-summary')"/>
        <xsl:variable name="list-width" select="f:pi(processing-instruction('dbhtml'),         'list-width')"/>
        <table border="0">
