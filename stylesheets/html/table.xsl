...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" exclude-result-prefixes="db doc f ghost h m t u xs" version="2.0">
    ...
    <xsl:template match="db:table">
        <xsl:call-template name="t:formal-object">
            <xsl:with-param name="placement" select="$formal.title.placement[self::db:table]/@placement"/>
    ...
    <xsl:template name="t:table-longdesc">
        ...
        <xsl:if test="$html.longdesc != 0 and $html.longdesc.link != 0                 and db:textobject[not(db:phrase)]">
...
<!-- CALS tables -->
  ...
  -->
    ...
    <xsl:template match="db:tgroup" name="db:tgroup" mode="m:cals">
        ...
        <xsl:variable name="summary" select="f:pi(processing-instruction('dbhtml'),'table-summary')"/>
        <xsl:variable name="cellspacing" select="f:pi(processing-instruction('dbhtml'),'cellspacing')"/>
        <xsl:variable name="cellpadding" select="f:pi(processing-instruction('dbhtml'),'cellpadding')"/>
        <table border="0">
            <xsl:choose>
      <!-- If there's a textobject/phrase for the table summary, use it -->
                <xsl:when test="$origtable/../db:textobject/db:phrase">
                    <xsl:attribute name="summary" select="xs:string($origtable/../db:textobject/db:phrase)"/>
                </xsl:when>
      ...
      <!-- Otherwise, forget the whole idea -->
            ...
            <xsl:if test="$cellspacing != '' or $table.html.cellspacing != ''">
                <xsl:attribute name="cellspacing" select="if ($cellspacing != '')                       then $cellspacing         else $table.html.cellspacing"/>
            </xsl:if>
            <xsl:if test="$cellpadding != '' or $table.html.cellpadding != ''">
                <xsl:attribute name="cellpadding" select="if ($cellpadding != '')                       then $cellpadding         else $table.html.cellpadding"/>
            </xsl:if>
            <xsl:if test="$origtable/../@pgwide=1 or self::db:entrytbl">
                <xsl:attribute name="width" select="'100%'"/>
            </xsl:if>
            <xsl:variable name="frame" select="if ($origtable/../@frame)                           then $origtable/../@frame      else $table.frame.default"/>
            <xsl:choose>
            ...
            <xsl:variable name="explicit.table.width" select="f:pi(processing-instruction('dbhtml'),'table-width')"/>
            ...
            <xsl:if test="string($table.width.default) != ''                   or $explicit.table.width != ''">
                <xsl:attribute name="width" select="f:convert-length($table.width)"/>
            </xsl:if>
            ...
            <xsl:if test=".//db:footnote">
                <tbody class="footnotes">
                    <tr>
                        <td colspan="{@cols}">
                            <xsl:apply-templates select="$origtable//db:footnote" mode="m:table-footnote-mode"/>
                        </td>
                    </tr>
                </tbody>
            </xsl:if>
    ...
    <xsl:template match="db:entrytbl" mode="m:cals">
        ...
        <xsl:variable name="cellgi">
            <xsl:choose>
                ...
                <xsl:when test="ancestor::db:tgroup/parent::*/@rowheader='firstcol'         and ghost:colnum=1">th</xsl:when>
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:cals">
  ...
  <!-- nop -->
    ...
    <xsl:template match="db:row" mode="m:cals">
        <xsl:param name="origtable" required="yes" as="element(db:tgroup)"/>
        <xsl:variable name="row-height" select="f:pi(processing-instruction('dbhtml'),'row-height')"/>
        <xsl:variable name="bgcolor" select="f:pi(processing-instruction('dbhtml'),'bgcolor')"/>
        <xsl:variable name="class" select="f:pi(processing-instruction('dbhtml'),'class')"/>
        <tr>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="tr-attributes">
...
to alternate rows of the table:</para>
  ...
  <!-- nop -->
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="m:cals">
        <xsl:copy/>
    </xsl:template>

<!-- ==================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="border">
...
<!-- ============================================================ -->
<!-- HTML tables -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:html">
    ...
    <xsl:template match="db:col|db:colgroup                      |db:thead|db:tfoot|db:tbody|db:tr        |db:th|db:td" mode="m:html">
        <xsl:element name="{local-name(.)}" namespace="http://www.w3.org/1999/xhtml">
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates mode="m:html"/>
        </xsl:element>
