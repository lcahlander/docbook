...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:ext="http://docbook.org/extensions/xslt20" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="db doc f ghost h m t u xlink xs ext" version="2.0">
...
<!-- ==================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:process-image">
...
vertical alignment.</para>
    ...
    <xsl:template name="t:image-properties" as="xs:integer*">
        ...
        <xsl:sequence use-when="function-available('ext:image-properties')" select="ext:image-properties($image)"/>
        ...
        <xsl:sequence use-when="not(function-available('ext:image-properties'))" select="()"/>
    </xsl:template>
    <xsl:function xmlns:magick="http://marklogic.com/magick" use-when="function-available('magick:wand')" name="ext:image-properties" as="xs:unsignedLong*">
  ...
  <!-- handle them all in one place.                             -->
        ...
        <xsl:variable xmlns:svg="http://www.w3.org/2000/svg" name="format" select="if (@format) then string(@format)                         else if (svg:*) then 'svg'                         else ''"/>
  ...
  <!-- imagedata, videodata, audiodata -->
        ...
        <xsl:variable name="intrinsicwidth" select="if (exists($imageproperties))                         then $imageproperties[1]                         else $nominal.image.width"/>
        <xsl:variable name="intrinsicdepth" select="if (exists($imageproperties))                         then $imageproperties[2]                         else $nominal.image.depth"/>
        ...
        <xsl:variable name="scaled.contentwidth">
            <xsl:if test="$contentwidth != ''">
                <xsl:variable name="cwidth.in.points" select="f:length-in-points($contentwidth,$points.per.em)"/>
        ...
        <xsl:variable name="html.width">
            <xsl:choose>
                ...
                <xsl:when test="$width != ''">
                    <xsl:variable name="width.in.points" select="f:length-in-points($width,$points.per.em)"/>
        ...
        <xsl:variable name="contentdepth">
            <xsl:choose>
                ...
                <xsl:when test="@contentdepth">
                    ...
                    <xsl:choose>
                        <xsl:when test="$units = '%'">
                            <xsl:variable name="cmagnitude" select="f:length-magnitude(@contentdepth)"/>
        ...
        <xsl:variable name="scaled.contentdepth">
            <xsl:if test="$contentdepth != ''">
                <xsl:variable name="cdepth.in.points" select="f:length-in-points($contentdepth,$points.per.em)"/>
        ...
        <xsl:variable name="html.depth">
            <xsl:choose>
                ...
                <xsl:when test="@depth and @depth != ''">
                    <xsl:variable name="depth.in.points" select="f:length-in-points($depth,$points.per.em)"/>
        ...
        <xsl:variable name="viewport">
            <xsl:choose>
                <xsl:when test="$ignore.image.scaling != 0">0</xsl:when>
                <xsl:when test="ancestor::db:inlinemediaobject                       or ancestor::db:inlineequation">0</xsl:when>
                <xsl:otherwise>
...
-->
        ...
        <xsl:variable name="href" select="f:mediaobject-href($filename)"/>
      ...
      <!-- attempt to handle audio data -->
          ...
          <!-- this is a complete hack; DocBook needs more markup -->
                ...
                <xsl:when xmlns:svg="http://www.w3.org/2000/svg" test="$format = 'svg' and svg:*">
                    <xsl:apply-templates/>
                </xsl:when>
                ...
                <xsl:otherwise>
                    <xsl:element name="{$tag}">
                        <xsl:if test="@role or ../@role">
                            <xsl:variable name="values" select="tokenize(concat(@role, ' ', ../@role), '\s+')"/>
        ...
        <xsl:variable name="bgcolor" select="f:pi(ancestor-or-self::db:imageobject/processing-instruction('dbhtml'),              'background-color')"/>
        ...
        <xsl:choose>
            <xsl:when test="$use.viewport">
                <table border="0" summary="Manufactured viewport for HTML image" cellspacing="0" cellpadding="0">
        ...
        <xsl:if test="$tag = 'img'                 and ancestor::db:imageobjectco                 and not($scaled)                 and exists($imageproperties)">
            <map name="{f:imagemap-name(ancestor::db:imageobjectco)}">
                <xsl:for-each select="ancestor::db:imageobjectco/db:areaspec//db:area">
                    <xsl:variable name="units" as="xs:string" select="if (@units) then @units                               else if (../@units) then ../@units                                    else 'calspair'"/>
                    <xsl:choose>
                        <xsl:when test="$units = 'calspair'">
                            <xsl:variable name="coords" select="tokenize(normalize-space(@coords),                                            '[\s,]+')"/>
                            ...
                            <area shape="rect">
                                <xsl:choose>
                                    <xsl:when test="@linkends                                 or (parent::db:areaset and ../@linkends)">
                                        <xsl:variable name="idrefs" select="if (@linkends)                                         then normalize-space(@linkends)                                         else normalize-space(../@linkends)"/>
       ...
       pass. It's probably overkill, but it might come in useful I suppose -->
        <xsl:choose>
            <xsl:when test="exists(f:pi($ioco/processing-instruction('dbhtml'), 'mapid'))">
                <xsl:value-of select="f:pi($ioco/processing-instruction('dbhtml'), 'mapid')"/>
            </xsl:when>
            <xsl:otherwise>
...
<!-- ==================================================================== -->
    <xsl:template match="db:mediaobject">
        ...
        <xsl:variable name="center" select="if ($object/*/@align = 'center') then 'centerimg' else ()"/>

  <!-- hack -->
        <xsl:choose>
            <xsl:when test="$object/self::db:audioobject                     and $object/db:audiodata/@condition='hidden'">
                ...
                <div>
                    ...
                    <xsl:if test="$html.longdesc != 0 and $html.longdesc.link != 0">
                        <xsl:call-template name="t:longdesc-link">
                            <xsl:with-param name="textobject" select="db:textobject[not(db:phrase)][1]"/>
                        </xsl:call-template>
                    </xsl:if>
                    <xsl:apply-templates select="$object"/>
    ...
    <xsl:template match="db:programlisting/db:inlinemediaobject                      |db:screen/db:inlinemediaobject" priority="2">
    ...
    <xsl:template match="db:imagedata">
        ...
        <xsl:choose>
            <xsl:when test="@format='linespecific' and $textdata.default.encoding != ''">
                <xsl:value-of select="unparsed-text($filename,                                           $textdata.default.encoding)"/>
            </xsl:when>
            <xsl:when test="@format='linespecific'">
                <xsl:value-of select="unparsed-text($filename)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:variable name="longdesc.uri" select="f:longdesc-uri(ancestor::db:imageobject/parent::*)"/>
                <xsl:variable name="phrases" select="../../db:textobject[db:phrase]"/>
                <xsl:variable name="normalphrases" select="$phrases[not(@role) or @role != 'tex']"/>
...
<!-- ==================================================================== -->
    ...
    <xsl:template match="db:textdata">
        ...
        <xsl:value-of select="if ($encoding = '')                         then unparsed-text($filename)                         else unparsed-text($filename, $encoding)"/>
    </xsl:template>
...
<!-- ============================================================ -->
    <xsl:function name="f:longdesc-uri" as="xs:string?">
        ...
        <xsl:if test="exists($node) and $html.longdesc != 0                 and $node/db:textobject[not(db:phrase)]">
    ...
    <xsl:template name="t:longdesc-link">
        ...
        <xsl:if test="exists($textobject) and $html.longdesc != 0">
            <xsl:variable name="this.uri" select="concat($base.dir, f:href-target-uri($textobject))"/>
            <xsl:variable name="href.to" select="f:trim-common-uri-paths(f:longdesc-uri($textobject),                                                   $this.uri)"/>
            <div class="longdesc-link">
                <xsl:text>[</xsl:text>
                <a href="{$href.to}" target="longdesc" title="Link to long description">D</a>
                <xsl:text>]</xsl:text>
            </div>
        </xsl:if>
    </xsl:template>
    <xsl:template name="t:write-longdesc">
        ...
        <xsl:variable name="firsttext" select="$mediaobject/db:textobject[not(db:phrase)][1]"/>
        <xsl:variable name="filename" select="f:longdesc-uri($firsttext)"/>
        <xsl:if test="$html.longdesc != 0                 and $mediaobject/db:textobject[not(db:phrase)]                 and $filename != ''">
