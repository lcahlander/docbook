...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn h m t u xlink xs" version="2.0">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="anchor">
...
not have an ID, make this parameter non-zero. It defaults to 0.</para>
        ...
        <u:unittests template="anchor">
            <u:test>
                ...
                <u:result>
                    <a xmlns="http://www.w3.org/1999/xhtml" name="foo" id="foo"/>
                </u:result>
            </u:test>
        </u:unittests>
    </doc:template>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="class">
...
and the node has no <tag class="attribute">role</tag>.</para>
    ...
    <xsl:template name="class">
        ...
        <xsl:variable name="role" select="if ($node/@role) then $node/@role else $default-role"/>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="style">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:javascript">
    ...
    <xsl:template name="t:system-javascript">
        ...
        <xsl:if test="//*[@xlink:type='extended']">
            ...
            <link type="text/css" rel="stylesheet" href="http://ajax.googleapis.com/ajax/libs/jqueryui/1.7.2/themes/start/jquery-ui.css"/>
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:css">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:head">
...
primary result document.</para>
    ...
    <xsl:template name="t:head">
        ...
        <head>
            ...
            <xsl:if test="($draft.mode = 'yes'                    or ($draft.mode = 'maybe' and          $node/ancestor-or-self::*[@status][1]/@status = 'draft'))                   and $draft.watermark.image != ''">
                <style type="text/css">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:head-meta">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:head-links">
...
HTML <tag>link</tag> elements.
    ...
    <xsl:template name="t:head-links">
        ...
        <xsl:if test="$link.madeby.uri != ''">
            <link rev="made" href="{$link.madeby.uri}"/>
        </xsl:if>
    </xsl:template>

<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:body-attributes">
...
<!-- ====================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:href">
...
<!-- ====================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:href-target-uri">
        <refpurpose>Returns a URI for the node</refpurpose>
        <refdescription>
            <para>This function returns a URI suitable for use in a hypertext link
for the specified node.</para>
...
<!-- ====================================================================== -->
    <xsl:function name="f:keep-titlepage-fragment" as="xs:boolean">
        ...
        <xsl:value-of select="string($fragment) != ''                         or count($fragment//h:div) != count($fragment//*)"/>
    </xsl:function>
...
<!-- ====================================================================== -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:strip-anchors">
...
is preserved, only the wrapping <tag>a</tag> is stripped away.</para>
    ...
    <xsl:template match="h:sup[@class='footnote']" mode="m:strip-anchors" priority="100">
  <!-- strip it -->
    </xsl:template>
    <xsl:template match="text()|comment()|processing-instruction()" mode="m:strip-anchors">
        <xsl:copy/>
    </xsl:template>
...
<!-- ====================================================================== -->
    <xsl:template match="*" mode="m:html-attributes" as="attribute()*">
        ...
        <xsl:variable name="value-seq" select="(if ($suppress-local-name-class)                          then () else local-name(.),                          if ($class != '') then $class else (),                          if ($suppress-role-class or not(@role))                          then ()                          else @role,                          if (@revisionflag)                          then concat('rf-',@revisionflag)                          else ())"/>
        <xsl:if test="not(empty($value-seq))">
            <xsl:attribute name="class" select="normalize-space(string-join($value-seq, ' '))"/>
        </xsl:if>
    </xsl:template>
...
<!-- ============================================================ -->
    <xsl:function name="f:html-class" as="attribute()*">
        ...
        <xsl:if test="exists($class) or exists($extra-classes)">
            <xsl:attribute name="class" select="normalize-space(string-join(($class,$extra-classes), ' '))"/>
        </xsl:if>
    </xsl:function>
