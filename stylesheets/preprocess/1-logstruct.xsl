...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="db f mp xs" version="2.0">
...
<!-- This may be the first in a series of transformations. Because
    ...
    <xsl:template match="db:imagedata[@entityref]                      |db:textdata[@entityref]        |db:videodata[@entityref]                      |db:audiodata[@entityref]" mode="mp:justcopy">
        <xsl:copy>
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="mp:justcopy">
        <xsl:copy/>
    </xsl:template>
</xsl:stylesheet>