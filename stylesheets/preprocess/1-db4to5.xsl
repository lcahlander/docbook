...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="db tp f mp" version="2.0">
...
<!-- Let the next phase deal with where space is insignificant -->
    ...
    <xsl:template match="bookinfo|chapterinfo|articleinfo|artheader|appendixinfo        |blockinfo                      |bibliographyinfo|glossaryinfo|indexinfo|setinfo        |setindexinfo                      |sect1info|sect2info|sect3info|sect4info|sect5info                      |sectioninfo                      |refsect1info|refsect2info|refsect3info|refsectioninfo        |referenceinfo|partinfo" priority="200" mode="mp:db4to5">
        <info>
    ...
    <xsl:template match="objectinfo|prefaceinfo|refsynopsisdivinfo        |screeninfo|sidebarinfo" priority="200" mode="mp:db4to5">
        <info>
            <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="refentryinfo" priority="200" mode="mp:db4to5">
        <info>
            <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="refmiscinfo" priority="200" mode="mp:db4to5">
        <refmiscinfo>
            <xsl:call-template name="copy.attributes">
            ...
            <xsl:if test="@class">
                <xsl:choose>
                    <xsl:when test="@class = 'source'           or @class = 'version'           or @class = 'manual'           or @class = 'sectdesc'           or @class = 'software'">
    ...
    <xsl:template match="corpauthor" priority="200" mode="mp:db4to5">
        <author>
            <xsl:call-template name="copy.attributes"/>
            <orgname>
                <xsl:apply-templates mode="mp:db4to5"/>
    ...
    <xsl:template match="corpname" priority="200" mode="mp:db4to5">
        <orgname>
            <xsl:call-template name="copy.attributes"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </orgname>
    </xsl:template>
    <xsl:template match="author[not(personname)]|editor[not(personname)]|othercredit[not(personname)]" priority="200" mode="mp:db4to5">
        <xsl:copy>
            <xsl:call-template name="copy.attributes"/>
            <personname>
                <xsl:apply-templates select="honorific|firstname|surname|othername|lineage" mode="mp:db4to5"/>
            </personname>
            <xsl:apply-templates select="*[not(self::honorific|self::firstname|self::surname                                    |self::othername|self::lineage)]" mode="mp:db4to5"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="address|programlisting|screen|funcsynopsisinfo                      |classsynopsisinfo|literallayout" priority="200" mode="mp:db4to5">
        <xsl:copy>
            <xsl:call-template name="copy.attributes">
    ...
    <xsl:template match="productname[@class]" priority="200" mode="mp:db4to5">
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
    ...
    <xsl:template match="dedication|preface|chapter|appendix|part|partintro                      |article|bibliography|glossary|glossdiv|index        |reference[not(referenceinfo)]|book" priority="200" mode="mp:db4to5">
        <xsl:choose>
            <xsl:when test="not(dedicationinfo|prefaceinfo|chapterinfo           |appendixinfo|partinfo                         |articleinfo|artheader|bibliographyinfo    |glossaryinfo|indexinfo                         |bookinfo)">
                <xsl:copy>
                    <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="formalpara|figure|table[tgroup]|example|blockquote                      |caution|important|note|warning|tip                      |bibliodiv|glossarydiv|indexdiv        |orderedlist|itemizedlist|variablelist|procedure        |task|tasksummary|taskprerequisites|taskrelated        |sidebar" priority="200" mode="mp:db4to5">
        <xsl:choose>
    ...
    <xsl:template match="equation" priority="200" mode="mp:db4to5">
        <xsl:choose>
            <xsl:when test="not(title)">
                <xsl:call-template name="tp:emit-message">
                    <xsl:with-param name="message">Convert equation without title to informal equation.</xsl:with-param>
                </xsl:call-template>
                <informalequation>
                    <xsl:call-template name="copy.attributes"/>
                    <xsl:apply-templates mode="mp:db4to5"/>
    ...
    <xsl:template match="sect1|sect2|sect3|sect4|sect5|section" priority="200" mode="mp:db4to5">
        <section>
            <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="simplesect" priority="200" mode="mp:db4to5">
        <simplesect>
            <xsl:call-template name="copy.attributes"/>
            <info>
                <xsl:apply-templates select="title" mode="mp:copy"/>
    ...
    <xsl:template match="refsect1|refsect2|refsect3|refsection" priority="200" mode="mp:db4to5">
        <refsection>
            <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="imagedata|videodata|audiodata|textdata" priority="200" mode="mp:db4to5">
        <xsl:copy>
            <xsl:call-template name="copy.attributes">
    ...
    <xsl:template match="sgmltag" priority="200" mode="mp:db4to5">
        <tag>
            <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="inlinegraphic[@format='linespecific']" priority="210" mode="mp:db4to5">
        <textobject>
            <textdata>
                <xsl:call-template name="copy.attributes"/>
            </textdata>
    ...
    <xsl:template match="inlinegraphic" priority="200" mode="mp:db4to5">
        <inlinemediaobject>
            <imageobject>
                <imagedata>
                    <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="graphic[@format='linespecific']" priority="210" mode="mp:db4to5">
        <mediaobject>
            <textobject>
                <textdata>
                    <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="graphic" priority="200" mode="mp:db4to5">
        <mediaobject>
            <imageobject>
                <imagedata>
                    <xsl:call-template name="copy.attributes"/>
    ...
    <xsl:template match="pubsnumber" priority="200" mode="mp:db4to5">
        <biblioid class="pubsnumber">
            <xsl:call-template name="copy.attributes"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </biblioid>
    </xsl:template>
    <xsl:template match="invpartnumber" priority="200" mode="mp:db4to5">
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
    ...
    <xsl:template match="contractsponsor" priority="200" mode="mp:db4to5">
        <xsl:variable name="contractnum" select="preceding-sibling::contractnum|following-sibling::contractnum"/>
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
    ...
    <xsl:template match="contractnum" priority="200" mode="mp:db4to5">
        <xsl:if test="not(preceding-sibling::contractsponsor                     |following-sibling::contractsponsor)                 and not(preceding-sibling::contractnum)">
            <xsl:call-template name="tp:emit-message">
                <xsl:with-param name="message">
            ...
            <othercredit class="other" otherclass="contractnum">
                <orgname>???</orgname>
                <xsl:for-each select="self::contractnum                             |preceding-sibling::contractnum                             |following-sibling::contractnum">
                    <contrib>
                        <xsl:apply-templates select="node()" mode="mp:db4to5"/>
                    </contrib>
                </xsl:for-each>
    ...
    <xsl:template match="isbn|issn" priority="200" mode="mp:db4to5">
        <biblioid class="{local-name(.)}">
            <xsl:call-template name="copy.attributes"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </biblioid>
    </xsl:template>
    <xsl:template match="biblioid[count(*) = 1                 and ulink          and normalize-space(text()) = '']" priority="200" mode="mp:db4to5">
        <biblioid xlink:href="{ulink/@url}">
            <xsl:call-template name="copy.attributes"/>
            <xsl:apply-templates select="ulink/node()" mode="mp:db4to5"/>
        </biblioid>
    </xsl:template>
    <xsl:template match="authorblurb" priority="200" mode="mp:db4to5">
        <personblurb>
            <xsl:call-template name="copy.attributes"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </personblurb>
    </xsl:template>
    <xsl:template match="collabname" priority="200" mode="mp:db4to5">
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
    ...
    <xsl:template match="modespec" priority="200" mode="mp:db4to5">
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
  ...
  <!-- get rid of any embedded markup -->
    ...
    <xsl:template match="biblioentry/title                      |bibliomset/title                      |biblioset/title                      |bibliomixed/title" priority="400" mode="mp:db4to5">
        <citetitle>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </citetitle>
    </xsl:template>
    <xsl:template match="biblioentry/titleabbrev|biblioentry/subtitle                      |bibliomset/titleabbrev|bibliomset/subtitle                      |biblioset/titleabbrev|biblioset/subtitle                      |bibliomixed/titleabbrev|bibliomixed/subtitle" priority="400" mode="mp:db4to5">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="biblioentry/contrib                      |bibliomset/contrib                      |bibliomixed/contrib" priority="200" mode="mp:db4to5">
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
        ...
        <xsl:choose>
            <xsl:when test="@targetdocent">
                ...
                <olink targetdoc="{unparsed-entity-uri(@targetdocent)}">
                    <xsl:for-each select="@*">
                        <xsl:if test="name(.) != 'targetdocent'                  and name(.) != 'linkmode'">
                            <xsl:copy/>
                        </xsl:if>
                    </xsl:for-each>
                    <xsl:apply-templates mode="mp:db4to5"/>
    ...
    <xsl:template match="biblioentry/firstname                      |biblioentry/surname                      |biblioentry/othername                      |biblioentry/lineage                      |biblioentry/honorific                      |bibliomset/firstname                      |bibliomset/surname                      |bibliomset/othername                      |bibliomset/lineage                      |bibliomset/honorific" priority="200" mode="mp:db4to5">
        <xsl:choose>
            <xsl:when test="preceding-sibling::firstname                     |preceding-sibling::surname                     |preceding-sibling::othername                     |preceding-sibling::lineage                     |preceding-sibling::honorific">
      <!-- nop -->
            </xsl:when>
            <xsl:otherwise>
                <personname>
                    <xsl:apply-templates select="../firstname                                      |../surname                                      |../othername                                      |../lineage                                      |../honorific" mode="mp:copy"/>
                </personname>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="date|pubdate" priority="200" mode="mp:db4to5">
        ...
        <xsl:variable name="rp2" select="substring-before(substring-after(normalize-space(.), ' '),                            ' ')"/>
        <xsl:variable name="rp3" select="substring-after(substring-after(normalize-space(.), ' '), ' ')"/>
  ...
  <!-- don't copy the defaulted significance='normal' attribute -->
    ...
    <xsl:template match="lot|lotentry|tocback|tocchap|tocfront|toclevel1|        toclevel2|toclevel3|toclevel4|toclevel5|tocpart" priority="200" mode="mp:db4to5">
        <tocdiv>
            <xsl:copy-of select="@*"/>
            <xsl:apply-templates mode="mp:db4to5"/>
        </tocdiv>
    ...
    <xsl:template match="beginpage" priority="200" mode="mp:db4to5">
        <xsl:comment> beginpage pagenum=<xsl:value-of select="@pagenum"/>
        </xsl:comment>
        <xsl:call-template name="tp:emit-message">
            <xsl:with-param name="message">
    ...
    <xsl:template name="copy.attributes">
        ...
        <xsl:for-each select="$src/@*">
            <xsl:choose>
                ...
                <xsl:when test="local-name($src) = 'simplemsgentry'                and local-name(.) = 'audience'">
                    <xsl:attribute name="msgaud">
                        <xsl:value-of select="."/>
                    </xsl:attribute>
                </xsl:when>
                <xsl:when test="local-name($src) = 'simplemsgentry'                and local-name(.) = 'origin'">
                    <xsl:attribute name="msgorig">
                        <xsl:value-of select="."/>
                    </xsl:attribute>
                </xsl:when>
                <xsl:when test="local-name($src) = 'simplemsgentry'                and local-name(.) = 'level'">
      ...
      <!-- * for upgrading XSL litprog params documentation -->
                <xsl:when test="local-name($src) = 'refmiscinfo'                       and local-name(.) = 'role'                       and . = 'type'                       ">
                    <xsl:call-template name="tp:emit-message">
                        <xsl:with-param name="message">
