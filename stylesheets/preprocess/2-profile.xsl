...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fp="http://docbook.org/xslt/ns/extension/private" exclude-result-prefixes="db doc f fp m mp xs" version="2.0">
...
<!-- This stylesheet profiles the DocBook document according to its
    ...
    <xsl:template match="/">
        ...
        <xsl:sequence select="f:profile(/,                                   $profile.separator, $profile.lang,                                   $profile.revisionflag, $profile.role,                                   $profile.arch,                                   $profile.audience,                                   $profile.condition,                                   $profile.conformance,                                   $profile.os,                                   $profile.outputformat,                                   $profile.revision,                                   $profile.security,                                   $profile.userlevel,                                   $profile.vendor,                                   $profile.wordsize                                  )"/>
    </xsl:template>
    ...
    <xsl:template match="*" mode="mp:profile">
        ...
        <xsl:param name="profile.vendor" select="''"/>
        <xsl:param name="profile.wordsize" select="''"/>
        <xsl:if test="fp:profile-ok(@lang, $profile.lang, $profile.separator)                 and fp:profile-ok(@revisionflag, $profile.revisionflag, $profile.separator)                 and fp:profile-ok(@role, $profile.role, $profile.separator)                 and fp:profile-ok(@arch, $profile.arch, $profile.separator)                 and fp:profile-ok(@audience, $profile.audience, $profile.separator)                 and fp:profile-ok(@condition, $profile.condition, $profile.separator)                 and fp:profile-ok(@conformance, $profile.conformance, $profile.separator)                 and fp:profile-ok(@os, $profile.os, $profile.separator)                 and fp:profile-ok(@outputformat, $profile.outputformat, $profile.separator)                 and fp:profile-ok(@revision, $profile.revision, $profile.separator)                 and fp:profile-ok(@security, $profile.security, $profile.separator)                 and fp:profile-ok(@userlevel, $profile.userlevel, $profile.separator)                 and fp:profile-ok(@vendor, $profile.vendor, $profile.separator)                 and fp:profile-ok(@wordsize, $profile.wordsize, $profile.separator)                 ">
            <xsl:copy>
                <xsl:copy-of select="@*"/>
                <xsl:apply-templates mode="mp:profile">
                    <xsl:with-param name="profile.separator" select="$profile.separator"/>
    ...
    <xsl:template match="text()|comment()|processing-instruction()" mode="mp:profile">
        <xsl:copy/>
    </xsl:template>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="fp:profile-ok">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="fp:profile-attribute-ok">
...
true if the current attribute is in the specified profile.</para>
    ...
    <xsl:function name="fp:profile-attribute-ok" as="xs:boolean">
        ...
        <xsl:choose>
            ...
            <xsl:when test="not($context/@*[local-name(.) = $attrname                       and namespace-uri(.) = ''])">
                <xsl:value-of select="true()"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="fp:profile-ok($context/@*[local-name(.) = $attrname                                                      and namespace-uri(.) = ''],       $prof, $profile.separator)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
</xsl:stylesheet>