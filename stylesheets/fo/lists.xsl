...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:fo="http://www.w3.org/1999/XSL/Format" exclude-result-prefixes="db f m t xs" version="2.0">
...
<!-- ==================================================================== -->
    <xsl:template match="db:itemizedlist|db:orderedlist">
        ...
        <xsl:variable name="pi-label-width" select="f:dbfo-pi-attribute(.,'label-width')"/>
  ...
  <!-- Preserve order of PIs and comments -->
        <xsl:apply-templates select="*[not(self::db:listitem)]               |comment()[not(preceding-sibling::listitem)]               |processing-instruction()[not(preceding-sibling::listitem)]"/>
        <xsl:variable name="content">
            <xsl:apply-templates select="db:listitem                   |comment()[preceding-sibling::listitem]                   |processing-instruction()[preceding-sibling::listitem]"/>
        </xsl:variable>
  ...
  <!-- nested lists don't add extra list-block spacing -->
        <xsl:choose>
            <xsl:when test="ancestor::db:listitem">
                <fo:list-block id="{$id}" xsl:use-attribute-sets="itemizedlist.properties">
                    <xsl:attribute name="provisional-distance-between-starts" select="$label-width"/>
    ...
    <xsl:template match="db:orderedlist/db:listitem" mode="m:listitem-number">
        <xsl:variable name="numeration" select="f:orderedlist-numeration(parent::db:orderedlist)"/>
      ...
      <!-- What!? This should never happen -->
        ...
        <xsl:variable name="item-number" select="f:orderedlist-item-number(.)"/>
        <xsl:if test="parent::db:orderedlist/@inheritnum='inherit'                 and ancestor::db:listitem[parent::db:orderedlist]">
            <xsl:apply-templates select="ancestor::db:listitem[parent::orderedlist][1]" mode="m:listitem-number"/>
        </xsl:if>
...
-->
    <xsl:template match="db:variablelist">
        <xsl:variable name="presentation" select="f:dbfo-pi-attribute(.,'list-presentation')"/>
        <xsl:choose>
    ...
    <xsl:template match="db:variablelist" mode="mp:vl.as.list">
        <xsl:variable name="id" select="f:node-id(.)"/>
        <xsl:variable name="term-width" select="f:dbfo-pi-attribute(.,'term-width')"/>
...
-->
  ...
  <!-- Preserve order of PIs and comments -->
        <xsl:apply-templates select="*[not(self::db:varlistentry)]             |comment()[not(preceding-sibling::db:varlistentry)]             |processing-instruction()[not(preceding-sibling::db:varlistentry)]"/>
        <xsl:variable name="content">
            <xsl:apply-templates mode="mp:vl.as.list" select="db:varlistentry               |comment()[preceding-sibling::db:varlistentry]               |processing-instruction()[preceding-sibling::db:varlistentry]"/>
        </xsl:variable>

  <!-- nested lists don't add extra list-block spacing -->
        <xsl:choose>
            <xsl:when test="ancestor::db:listitem">
                <fo:list-block id="{$id}" provisional-distance-between-starts="{$distance-between-starts}" provisional-label-separation="{$label-separation}">
                    <xsl:copy-of select="$content"/>
                </fo:list-block>
            </xsl:when>
            <xsl:otherwise>
                <fo:list-block id="{$id}" provisional-distance-between-starts="{$distance-between-starts}" provisional-label-separation="{$label-separation}" xsl:use-attribute-sets="list.block.spacing">
                    <xsl:copy-of select="$content"/>
                </fo:list-block>
            </xsl:otherwise>
        </xsl:choose>
  ...
  <!-- Preserve order of PIs and comments -->
        <xsl:apply-templates select="*[not(self::db:varlistentry)]             |comment()[not(preceding-sibling::db:varlistentry)]             |processing-instruction()[not(preceding-sibling::db:varlistentry)]"/>
        <xsl:variable name="content">
            <xsl:apply-templates mode="mp:vl.as.blocks" select="db:varlistentry               |comment()[preceding-sibling::db:varlistentry]               |processing-instruction()[preceding-sibling::db:varlistentry]"/>
        </xsl:variable>
  ...
  <!-- nested lists don't add extra list-block spacing -->
    ...
    <xsl:template match="db:varlistentry" mode="mp:vl.as.blocks">
        <xsl:variable name="id" select="f:node-id(.)"/>
        <fo:block id="{$id}" xsl:use-attribute-sets="list.item.spacing" keep-together.within-column="always" keep-with-next.within-column="always">
            <xsl:apply-templates select="db:term"/>
        </fo:block>
...
<!-- ==================================================================== -->
  ...
  <!-- with no type specified, the default is 'vert' -->
        <xsl:variable name="explicit.table.width" select="f:dbfo-pi-attribute(.,'list-width')"/>
...
<!-- ==================================================================== -->
    <xsl:template match="db:procedure">
        ...
        <xsl:call-template name="t:semiformal-object">
            <xsl:with-param name="placement" select="$formal.title.placement[self::db:procedure]/@placement"/>
            <xsl:with-param name="object" as="element()">
                ...
                <fo:list-block xsl:use-attribute-sets="list.block.spacing" provisional-distance-between-starts="{$procedure.label.width}">
