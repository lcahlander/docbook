...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:css="https://github.com/docbook/css-rules" exclude-result-prefixes="db doc f ghost h m t u xs html css" version="2.0">
    ...
    <xsl:template match="db:tgroup" name="db:tgroup" mode="m:cals">
        ...
        <fo:table>
            ...
            <xsl:call-template name="t:table-frame">
                <xsl:with-param name="frame" select="if ($origtable/../@frame)         then $origtable/../@frame         else $table.frame.default"/>
            </xsl:call-template>
            ...
            <xsl:variable name="explicit.table.width" select="f:pi(processing-instruction('dbfo'),'table-width')"/>
...
<!-- ============================================================ -->
  ...
  <!-- nop -->
    ...
    <xsl:template match="db:row" mode="m:cals">
        <xsl:param name="origtable" required="yes" as="element(db:tgroup)"/>
        <xsl:variable name="row-height" select="f:pi(processing-instruction('dbfo'),'row-height')"/>
        <xsl:variable name="bgcolor" select="f:pi(processing-instruction('dbfo'),'bgcolor')"/>
        <fo:table-row>
...
<!-- ============================================================ -->
    <xsl:template name="t:process-html-rows">
        ...
        <xsl:apply-templates select="$rows[1]" mode="m:html">
            <xsl:with-param name="cells" select="$rowdata[(position() mod 2 = 1)                                      and (namespace-uri(.) != 'http://docbook.org/ns/docbook/ephemeral')]"/>
        </xsl:apply-templates>
    ...
    <xsl:template name="t:process-row">
        ...
        <xsl:choose>
            <xsl:when test="exists($overhang) and $overhang[1] &gt; 0">
                <ghost:overlap/>
                <ghost:overhang>
                    <xsl:value-of select="$overhang[1] - 1"/>
                </ghost:overhang>
                <xsl:call-template name="t:process-row">
    ...
    <xsl:template name="t:parse-css">
        ...
        <xsl:for-each select="tokenize($style, '\s*;\s*')">
            ...
            <xsl:if test="$property != '' and $value != '' and exists($map)">
                <xsl:attribute name="{if ($map/@fo) then $map/@fo else $property}" select="$value"/>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>
