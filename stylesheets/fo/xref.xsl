...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn h m mp t u xlink xs" version="2.0">
...
<!-- ============================================================ -->
  ...
  <!-- is the @xlink:href a local idref link? -->
        <xsl:param name="xlink.idref" select="if (f:xpointer-idref($xhref) != '')                      then f:xpointer-idref($xhref)        else ''"/>
        ...
        <xsl:call-template name="t:check-id-unique">
            <xsl:with-param name="linkend" select="if ($xlink.idref != '')                             then $xlink.idref        else @linkend"/>
        </xsl:call-template>
  ...
  <!-- Add standard page reference? -->
    ...
    <!-- positive xrefstyle already handles it -->
            <xsl:when test="not(starts-with(normalize-space($xrefstyle), 'select:')        and (contains($xrefstyle, 'page')                          or contains($xrefstyle, 'Page')))                     and ($insert.xref.page.number = 'yes'                           or $insert.xref.page.number = '1')                     or $target/self::db:para">
...
<!--- ==================================================================== -->
  ...
  <!-- for title, xref to the parent (parent of the info, that is) -->
    ...
    <xsl:template match="db:abstract|db:article|db:bibliodiv|db:bibliomset                      |db:biblioset|db:blockquote|db:calloutlist|db:colophon                      |db:constraintdef|db:formalpara|db:glossdiv|db:indexdiv                      |db:itemizedlist|db:legalnotice|db:msg|db:msgexplan|db:msgmain                      |db:msgrel|db:msgset|db:msgsub|db:orderedlist|db:partintro                      |db:productionset|db:qandadiv|db:refsynopsisdiv|db:segmentedlist                      |db:set|db:setindex|db:sidebar|db:toc|db:variablelist" mode="m:xref-to">
        <xsl:param name="referrer"/>
        <xsl:param name="xrefstyle"/>
        <xsl:param name="verbose" select="1"/>
  ...
  <!-- catch-all for things with (possibly optional) titles -->
    ...
    <xsl:template match="db:caution|db:important|db:note|db:tip|db:warning" mode="m:xref-to">
        <xsl:param name="referrer"/>
        <xsl:param name="xrefstyle"/>
        <xsl:param name="verbose" select="1"/>
  ...
  <!-- catch-all for things with (possibly optional) titles -->
    ...
    <xsl:template match="db:dedication|db:preface|db:chapter|db:appendix" mode="m:xref-to">
        <xsl:param name="referrer"/>
        <xsl:param name="xrefstyle"/>
        <xsl:param name="verbose" select="1"/>
        <xsl:apply-templates select="." mode="m:object-xref-markup">
...
<!-- ==================================================================== -->
  ...
  <!-- Add standard page reference? -->
    ...
    <!-- There is no link element in DB5 with xlink:href -->
      ...
      <!-- negative xrefstyle in instance turns it off -->
            ...
            <xsl:when test="(starts-with(normalize-space($xrefstyle), 'select:')                       and $insert.link.page.number = 'maybe'                        and (contains($xrefstyle, 'page')                           or contains($xrefstyle, 'Page')))                      or ($insert.link.page.number = 'yes'                           or $insert.link.page.number = '1')                      or $target/self::db:para">
                <xsl:apply-templates select="$target" mode="m:page-citation">
  ...
  <!-- This template is called for all external hyperlinks;
       ...
       and how to display it (either inline or as a numbered footnote). -->
        <xsl:param name="url"/>
        <xsl:if test="not(empty(child::node()))   and string(.) != $url   and $ulink.show != 0">
    ...
    <xsl:template name="t:ulink-footnote-number">
      ...
      <!-- FIXME: list in @from is probably not complete -->
                <xsl:number level="any" from="db:chapter|db:appendix|db:preface|db:article    |db:refentry|db:bibliography[not(parent::db:article)]" count="db:footnote[not(@label)][not(ancestor::db:tgroup)]     |db:link[node()][@xlink:href != .][not(ancestor::db:footnote)]     |*[node()][@xlink:href][not(@xlink:href = .)][not(starts-with(@xlink:href,'#'))]                     [not(contains(@xlink:href,'#') and @xlink:role = $xolink.role)]                     [not(@xlink:type) or @xlink:type='simple']                     [not(ancestor::db:footnote)]" format="1"/>
            </xsl:variable>
            <xsl:choose>
...
<!-- ==================================================================== -->
    <xsl:template name="t:title-xref">
        ...
        <xsl:choose>
            <xsl:when test="$target/self::db:figure                     or $target/self::db:example                     or $target/self::db:equation                     or $target/self::db:table                     or $target/self::db:dedication                     or $target/self::db:preface                     or $target/self::db:bibliography                     or $target/self::db:glossary                     or $target/self::db:index                     or $target/self::db:setindex                     or $target/self::db:colophon">
