...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" exclude-result-prefixes="db doc f ghost h m t u xs" version="2.0">
    ...
    <!-- informaltables have their own templates and 
         are not handled by formal.object -->
            <xsl:when test="self::db:equation">
                <xsl:choose>
                    <xsl:when test="$pgwide = '1'">
                        <fo:block id="{$id}" xsl:use-attribute-sets="pgwide.properties                                             equation.properties">
                            <xsl:if test="$keep.together != ''">
            ...
            <xsl:when test="self::db:informalexample">
                <xsl:choose>
                    <xsl:when test="$pgwide = '1'">
                        <fo:block id="{$id}" xsl:use-attribute-sets="pgwide.properties                                             informalexample.properties">
                            <xsl:if test="$keep.together != ''">
                                <xsl:attribute name="keep-together.within-column">
                                    <xsl:value-of select="$keep.together"/>
                                </xsl:attribute>
            ...
            <xsl:when test="self::db:informalequation">
                <xsl:choose>
                    <xsl:when test="$pgwide = '1'">
                        <fo:block id="{$id}" xsl:use-attribute-sets="pgwide.properties                                             informalequation.properties">
                            <xsl:if test="$keep.together != ''">
                                <xsl:attribute name="keep-together.within-column">
                                    <xsl:value-of select="$keep.together"/>
                                </xsl:attribute>
  ...
  <!-- Get align value from internal mediaobject -->
        <xsl:variable name="align">
            <xsl:if test="db:mediaobject|db:mediaobjectco">
                <xsl:variable name="olist" select="db:mediaobject/db:imageobject                      |db:mediaobjectco/db:imageobjectco                      |db:mediaobject/db:videoobject                      |db:mediaobject/db:audioobject                      |db:mediaobject/db:textobject"/>
...
-->
    <xsl:template match="db:table|db:informaltable">
        <xsl:if test="db:tgroup/db:tbody/db:tr                 |db:tgroup/db:thead/db:tr                 |db:tgroup/db:tfoot/db:tr">
            <xsl:message terminate="yes">
                <xsl:text>Broken table: tr descendent of CALS Table.</xsl:text>
                <xsl:text>The text in the first tr is:
</xsl:text>
                <xsl:value-of select="(db:tgroup//db:tr)[1]"/>
            </xsl:message>
        </xsl:if>
  ...
  <!-- float or not -->
    ...
    <xsl:template match="db:equation">
        <xsl:variable name="param.placement" select="substring-after(normalize-space($formal.title.placement[self::db:equation]/@placement),                                       concat(local-name(.), ' '))"/>
