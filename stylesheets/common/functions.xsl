...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fp="http://docbook.org/xslt/ns/extension/private" exclude-result-prefixes="db doc f fp l m mp u xs" version="2.0">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:node-id">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:xptr-id">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:orderedlist-starting-number">
...
is to be determined.</para>
    ...
    <xsl:function name="f:orderedlist-starting-number" as="xs:integer">
        ...
        <xsl:variable name="pi-start" select="f:pi($list/processing-instruction('db-xsl'), 'start')"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="prevlist" select="$list/preceding::db:orderedlist[1]"/>
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:variable name="prevstart" select="f:orderedlist-starting-number($prevlist)"/>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:next-orderedlist-numeration">
...
is the first style in that list.</para>
    ...
    <xsl:function name="f:next-orderedlist-numeration" as="xs:string">
        ...
        <xsl:variable name="pos" select="index-of($orderedlist.numeration.styles,                                             $numeration)[1]"/>
        <xsl:choose>
            ...
            <xsl:when test="subsequence($orderedlist.numeration.styles,$pos+1,1)">
                <xsl:value-of select="subsequence($orderedlist.numeration.styles,                    $pos+1,1)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$orderedlist.numeration.styles[1]"/>
            </xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:orderedlist-numeration">
        <refpurpose>Returns the next numeration style</refpurpose>
        <refdescription>
    ...
    <xsl:function name="f:orderedlist-numeration" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$list/ancestor::db:orderedlist">
                <xsl:variable name="prevnumeration" select="f:orderedlist-numeration($list/ancestor::db:orderedlist[1])"/>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:orderedlist-item-number">
        <refpurpose>Returns the item number of the specified <tag>listitem</tag>
in an <tag>orderedlist</tag>
        </refpurpose>
        <refdescription>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:next-itemizedlist-symbol">
...
is the first style in that list.</para>
    ...
    <xsl:function name="f:next-itemizedlist-symbol" as="xs:string">
        ...
        <xsl:variable name="pos" select="index-of($itemizedlist.numeration.symbols,                                             $symbol)[1]"/>
        <xsl:choose>
            <xsl:when test="not($pos)">
                <xsl:value-of select="$itemizedlist.numeration.symbols[1]"/>
            </xsl:when>
            <xsl:when test="subsequence($itemizedlist.numeration.symbols,$pos+1,1)">
                <xsl:value-of select="subsequence($itemizedlist.numeration.symbols,                    $pos+1,1)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$itemizedlist.numeration.symbols[1]"/>
            </xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:itemizedlist-symbol">
        <refpurpose>Returns the mark that should be used for the specified
<tag>listitem</tag> or <tag>itemizedlist</tag>
        </refpurpose>
        <refdescription>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:procedure-step-numeration">
...
or <tag>steps</tag>.</para>
    ...
    <xsl:function name="f:procedure-step-numeration" as="xs:string">
        ...
        <xsl:variable name="format.length" select="count($procedure.step.numeration.styles)"/>
        <xsl:choose>
            <xsl:when test="$context/self::db:substeps">
                <xsl:variable name="ssdepth" select="count($context/ancestor::db:substeps)"/>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="subsequence($procedure.step.numeration.styles,                 $sstype,1)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:when test="$context/self::db:step">
                <xsl:variable name="sdepth" select="count($context/ancestor::db:substeps)"/>
                <xsl:variable name="stype" select="($sdepth mod $format.length)+1"/>
                <xsl:value-of select="subsequence($procedure.step.numeration.styles,                    $stype,1)"/>
            </xsl:when>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:in-scope-pis">
...
<emphasis>before</emphasis> the root node of the document.</para>
    ...
    <xsl:function name="f:in-scope-pis" as="processing-instruction()*">
        ...
        <xsl:sequence select="($context/processing-instruction()[local-name(.) = $target],                          root($context)/processing-instruction()[local-name(.) = $target])"/>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:pi">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:xpath-location">
...
<!-- ====================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:is-component">
...
elements to the list of components, if necessary.</para>
    ...
    <xsl:function name="f:is-component" as="xs:boolean">
        <xsl:param name="node" as="element()"/>
        <xsl:value-of select="if ($node/self::db:appendix        | $node/self::db:article        | $node/self::db:chapter        | $node/self::db:preface        | $node/self::db:bibliography        | $node/self::db:glossary        | $node/self::db:index)    then true()    else false()"/>
    </xsl:function>

<!-- ====================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:is-section">
...
is a <tag>section</tag>, <tag>sect<replaceable>1-5</replaceable>
                </tag>,
<tag>refsect<replaceable>1-3</replaceable>
                </tag>, or
...
elements to the list of sections, if necessary.</para>
    ...
    <xsl:function name="f:is-section" as="xs:boolean">
        <xsl:param name="node" as="element()"/>
        <xsl:value-of select="if ($node/self::db:section        | $node/self::db:sect1        | $node/self::db:sect2        | $node/self::db:sect3        | $node/self::db:sect4        | $node/self::db:sect5        | $node/self::db:refsect1        | $node/self::db:refsect2        | $node/self::db:refsect3        | $node/self::db:refsection        | $node/self::db:simplesect)     then true()     else false()"/>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:label-this-section">
...
<parameter>section.autolabel.max.depth</parameter>.</para>
    ...
    <xsl:function name="f:label-this-section" as="xs:boolean">
      ...
      <!-- If sections aren't being auto-labelled, then the answer is always false. -->
            ...
            <xsl:when test="f:section-level($section)       &lt;= $section.autolabel.max.depth">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:section-level">
    ...
    <xsl:function name="f:section-level" as="xs:integer">
        ...
        <xsl:variable name="level" as="xs:integer">
            <xsl:choose>
                ...
                <xsl:when test="$node/self::db:refsect1 or         $node/self::db:refsect2 or         $node/self::db:refsect3 or         $node/self::db:refsection or         $node/self::db:refsynopsisdiv">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:pseudo-section-level">
...
of elements such as <tag>qanda</tag>.</para>
    ...
    <xsl:function name="f:pseudo-section-level" as="xs:integer">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="section" select="($node/ancestor::db:section        |$node/ancestor::db:simplesect        |$node/ancestor::db:sect5        |$node/ancestor::db:sect4        |$node/ancestor::db:sect3        |$node/ancestor::db:sect2        |$node/ancestor::db:sect1        |$node/ancestor::db:refsection        |$node/ancestor::db:refsect3        |$node/ancestor::db:refsect2        |$node/ancestor::db:refsect1)[last()]"/>
                <xsl:choose>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:refentry-section-level">
...
enclosing <tag>refentry</tag>; see <function>refentry-level</function>.
    ...
    <xsl:function name="f:refentry-section-level" as="xs:integer">
        <xsl:param name="section" as="element()"/>
        <xsl:variable name="RElevel" select="f:refentry-level($section/ancestor::db:refentry[1])"/>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:refentry-level">
        <refpurpose>Returns the nesting depth of the specified <tag>refentry</tag>
        </refpurpose>
        <refdescription>
...
be calculated.</para>
    ...
    <xsl:function name="f:refentry-level" as="xs:integer">
        ...
        <xsl:variable name="container" select="($refentry/ancestor::db:section                          | $refentry/ancestor::db:sect1     | $refentry/ancestor::db:sect2     | $refentry/ancestor::db:sect3     | $refentry/ancestor::db:sect4     | $refentry/ancestor::db:sect5)[last()]"/>
        <xsl:choose>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:group-index">
...
This number is used to group index terms and to define order of groups.</para>
    ...
    <xsl:function name="f:group-index">
        ...
        <xsl:variable name="letters" as="element()*">
            <xsl:variable name="l10n.letters" select="($localization     //l:l10n[@language=$lang]     /l:letters,                f:load-locale($lang)/l:letters)[1]"/>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:copy-of select="($localization     //l:l10n[@language='en']     /l:letters,     f:load-locale('en')/l:letters)[1]"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:group-label">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:pad">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:find-node-in-sequence">
...
for another node with the same name to appear in the sequence.</para>
    ...
    <xsl:function name="f:find-node-in-sequence" as="xs:integer">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="f:find-node-in-sequence($nodes, $target,                                $start+1)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:find-toc-params">
...
matching path.</para>
    ...
    <xsl:function name="fp:find-toc-params" as="element()?">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:sequence select="fp:find-toc-params($node, $toc,             substring-after($location, '/'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:trim-common-uri-paths">
...
components that it has in common with <parameter>uriB</parameter>.</para>
    ...
    <xsl:function name="f:trim-common-uri-paths" as="xs:string">
        ...
        <xsl:choose>
            ...
            <xsl:when test="not(contains($uriA, '/'))                     or not(contains($uriB, '/'))       or substring-before($uriA, '/')           != substring-before($uriB, '/')">
                <xsl:value-of select="$uriA"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="f:trim-common-uri-paths(            substring-after($uriA, '/'),            substring-after($uriB, '/'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:filename-basename">
...
<!-- ============================================================ -->
    <xsl:function name="f:length-magnitude" as="xs:decimal?">
        ...
        <xsl:if test="matches(normalize-space($length), '^[0-9\.]+')">
            <xsl:value-of select="replace(normalize-space($length),              '(^[0-9\.]+).*',       '$1')"/>
        </xsl:if>
    </xsl:function>
...
<!-- ================================================================== -->
    ...
    <xsl:function name="f:length-units">
        ...
        <xsl:variable name="units" select="if (matches(normalize-space($length), '^[0-9\.]+.*?'))                         then replace(normalize-space($length),'^[0-9\.]+(.*?)$','$1')                         else ''"/>
        <xsl:choose>
            <xsl:when test="$units = ''">
...
<!-- ================================================================== -->
    <xsl:function name="f:first-in-context" as="xs:boolean">
        <xsl:param name="node" as="element()"/>
        <xsl:param name="context" as="element()"/>
        <xsl:variable name="pnode" select="$node/preceding::*[node-name(.)=node-name($node)][1]"/>
        <xsl:value-of select="if ($pnode)    then ($pnode &lt;&lt; $context) or ($pnode eq $context)    else true()"/>
    </xsl:function>
...
<!-- ================================================================== -->
    <xsl:function name="f:fake-eval-avt" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="contains($value,'{')">
                ...
                <xsl:variable name="var" select="substring-after(substring-before($value,'}'),'{')"/>
    ...
    <xsl:function name="f:lineNumbering">
        ...
        <xsl:variable name="params" select="f:find-toc-params($context, $linenumbering)"/>
        <xsl:variable name="cfgval" select="if ($params/@*[local-name(.) = $name])                         then $params/@*[local-name(.) = $name]                         else ''" as="xs:string?"/>
        <xsl:variable name="pival" select="f:pi($pis, concat('linenumbering.', $name))" as="xs:string?"/>
...
<!-- ================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:title">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:biblioentry-label">
    ...
    <xsl:template match="*" mode="mp:biblioentry-label-count">
        <xsl:choose>
            <xsl:when test="ancestor::db:bibliolist">
                <xsl:number from="db:bibliolist" count="db:biblioentry|db:bibliomixed" level="any" format="1"/>
            </xsl:when>
            <xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:xpointer-idref">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:verbatim-trim-blank-lines">
...
lines should be removed from the specified verbatim environment.</para>
    ...
    <xsl:function name="f:verbatim-trim-blank-lines" as="xs:boolean">
        ...
        <xsl:variable name="trim" select="f:pi(f:in-scope-pis($verbatim, 'dbhtml'), 'trim-verbatim')"/>
        <xsl:choose>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:dir">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:resolve-path">
...
an absolute URI (including a scheme!).</para>
    ...
    <xsl:function name="f:resolve-path" as="xs:string">
      ...
      <!-- otherwise, resolve them together -->
                ...
                <xsl:variable name="allsegs" select="(tokenize(substring-after($base, '/'), '/'),                                          tokenize($uri, '/'))"/>
    ...
    <xsl:function name="fp:resolve-dotdots" as="xs:string*">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:sequence select="fp:resolve-dotdots(                             (subsequence($segs, 1, $pos[1] - 2),                              subsequence($segs, $pos[1] + 1)))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
