...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:n="http://docbook.org/xslt/ns/normalize" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn m mp n xlink xs ghost" version="2.0">
    <xsl:param name="schemafile" select="''"/>

<!--	   select="'../../docbook/relaxng/dita4db/dita4db.rng'"/> -->
    <xsl:param name="schema" select="if ($schemafile = '')      then ()      else document($schemafile)"/>
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:normalize">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="n:normalize-movetitle">
        <refpurpose>Moves titles inside <tag>info</tag>
        </refpurpose>
        <refdescription>
...
<tag>titleabbrev</tag> elements inside an <tag>info</tag>.
                ...
                <xsl:when test="db:title|db:subtitle|db:titleabbrev">
                    <xsl:element name="info" namespace="{$docbook-namespace}">
                        <xsl:call-template name="n:normalize-dbinfo">
                            <xsl:with-param name="copynodes" select="db:title|db:subtitle|db:titleabbrev"/>
                        </xsl:call-template>
                    </xsl:element>
                    <xsl:apply-templates mode="m:normalize"/>
                </xsl:when>
    ...
    <xsl:template match="db:title|db:subtitle|db:titleabbrev" mode="m:normalize">
        <xsl:if test="parent::db:info                 |parent::db:biblioentry                 |parent::db:bibliomixed                 |parent::db:bibliomset                 |parent::db:biblioset">
            <xsl:copy>
                <xsl:copy-of select="@*"/>
                <xsl:apply-templates mode="m:normalize"/>
            </xsl:copy>
            ...
            <xsl:when test="not(node())"> <!-- totally empty -->
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$external.bibliography/key('id', $id)">
                        <xsl:apply-templates select="$external.bibliography/key('id', $id)" mode="m:normalize"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:message>
                            <xsl:text>Error: </xsl:text>
                ...
                <xsl:element name="glossary" namespace="{$docbook-namespace}">
                    ...
                    <xsl:variable name="seealsos" as="element()*">
                        <xsl:for-each select="$external.glossary//db:glossseealso">
                            <xsl:copy-of select="if (key('id', @otherterm))       then key('id', @otherterm)[1]       else key('glossterm', string(.))"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:variable name="divs" select="$glossary//db:glossary/db:glossdiv"/>
                    <xsl:choose>
                        <xsl:when test="$divs and $external.glossary//db:glossdiv">
                            <xsl:apply-templates select="$external.glossary//db:glossdiv" mode="m:copy-external-glossary">
                                <xsl:with-param name="terms" select="//db:glossterm[not(parent::db:glossdef)]           |//db:firstterm           |$seealsos"/>
                                <xsl:with-param name="divs" select="$divs"/>
                            </xsl:apply-templates>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:apply-templates select="$external.glossary//db:glossentry" mode="m:copy-external-glossary">
                                <xsl:with-param name="terms" select="//db:glossterm[not(parent::db:glossdef)]           |//db:firstterm           |$seealsos"/>
                                <xsl:with-param name="divs" select="$divs"/>
                            </xsl:apply-templates>
                        </xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="n:normalize-generated-title">
                ...
                <xsl:copy>
                    ...
                    <xsl:choose>
                        <xsl:when test="db:info">
                            ...
                            <xsl:apply-templates select="db:info/following-sibling::node()" mode="m:normalize"/>
                        </xsl:when>
                        <xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="n:normalize-dbinfo">
...
<tag>info</tag> element, then the rest of its contents are also normalized.
    ...
    <xsl:template match="db:inlinemediaobject        [(parent::db:programlisting          or parent::db:screen          or parent::db:literallayout          or parent::db:address          or parent::db:funcsynopsisinfo)        and db:imageobject        and db:imageobject/db:imagedata[@format='linespecific']]" mode="m:normalize">
        <xsl:variable name="data" select="(db:imageobject     /db:imagedata[@format='linespecific'])[1]"/>
        <xsl:choose>
    ...
    <xsl:template match="db:textobject        [parent::db:programlisting         or parent::db:screen         or parent::db:literallayout         or parent::db:address         or parent::db:funcsynopsisinfo]" mode="m:normalize">
        <xsl:choose>
...
<!-- Verbatim environments are normalized here too -->
    <xsl:template match="db:programlisting|db:address|db:screen|db:synopsis|db:literallayout" mode="m:normalize">
    ...
    <xsl:template xmlns:r="http://nwalsh.com/xmlns/schema-remap/" xmlns:rng="http://relaxng.org/ns/structure/1.0" match="*" mode="m:normalize">
        <xsl:variable name="element" select="local-name(.)"/>
        <xsl:variable name="element-name" select="node-name(.)"/>
        <xsl:variable name="known" select="for $n in $schema-extensions          return             if (node-name($n) = $element-name)      then $n      else ()"/>
  ...
  <!-- There are some limitations here with multiple patterns that define
       ...
       We can come back to them if they ever matter. -->
        <xsl:variable name="remap" select="$schema    //rng:element[@name=$element and r:remap]/r:remap[1]"/>
  ...
  -->
        <xsl:choose>
            ...
            <xsl:when test="$remap">
                <xsl:variable name="mapped" as="element()">
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <xsl:element name="{local-name($remap/*[1])}" namespace="{namespace-uri($remap/*[1])}">
            ...
            <xsl:when test="db:title|db:subtitle|db:titleabbrev|db:info/db:title">
                <xsl:choose>
                    <xsl:when test="parent::db:biblioentry                         |parent::db:bibliomixed                         |parent::db:bibliomset                         |parent::db:biblioset">
                        <xsl:copy>
                            <xsl:copy-of select="@*"/>
                            <xsl:apply-templates mode="m:normalize"/>
                        </xsl:copy>
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="m:normalize">
        <xsl:copy/>
    </xsl:template>

<!-- ============================================================ -->
    <xsl:template xmlns:r="http://nwalsh.com/xmlns/schema-remap/" match="r:content" mode="m:remap">
...
<!-- copy external glossary -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:copy-external-glossary">
...
<parameter>glossary.collection</parameter> into the current document.</para>
    ...
    <xsl:template match="db:glossentry" mode="m:copy-external-glossary">
        <xsl:param name="terms"/>
        <xsl:param name="divs"/>
        <xsl:variable name="include" select="for $dterm in $terms                            return                                for $gterm in db:glossterm                                  return                                     if (string($dterm) = string($gterm)                                         or $dterm/@baseform = string($gterm))                                     then 'x'                                     else ()"/>
        <xsl:if test="$include != ''">
            <xsl:copy-of select="."/>
        </xsl:if>
    </xsl:template>
