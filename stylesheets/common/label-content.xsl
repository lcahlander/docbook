...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f m fn xs" version="2.0">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:intralabel-punctuation">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:label-content">
...
processed in this mode should generate their label.</para>
  ...
  <!-- no label -->
    ...
    <xsl:template match="db:preface|db:chapter|db:appendix" mode="m:label-content">
        <xsl:variable name="label" select="$autolabel.elements          /*[node-name(.) = node-name(current())]"/>
        <xsl:variable name="format" as="xs:string">
            <xsl:choose>
                <xsl:when test="$label/@format">
                    <xsl:value-of select="$label/@format"/>
  ...
  -->
        <xsl:choose>
            ...
            <xsl:when test="$label">
                <xsl:if test="$component.label.includes.part.label != 0 and       ancestor::db:part">
                    <xsl:variable name="part.label">
                        <xsl:apply-templates select="ancestor::db:part" mode="m:label-content"/>
                    </xsl:variable>
                    <xsl:if test="$part.label != ''">
                        <xsl:value-of select="$part.label"/>
                        <xsl:apply-templates select="ancestor::db:part" mode="m:intralabel-punctuation"/>
                    </xsl:if>
                </xsl:if>
                <xsl:choose>
    ...
    <!-- if this is a nested section, label the parent -->
            ...
            <xsl:if test="$section.label.includes.component.label != 0     and parent::* instance of element()     and f:is-component(..)">
  ...
  <!-- yes: it's /db:section in the expression below -->
    ...
    <!-- if the parent is a component, maybe label that too -->
            <xsl:if test="$section.label.includes.component.label != 0     and f:is-component(..)">
                <xsl:variable name="parent.label">
                    <xsl:apply-templates select=".." mode="m:label-content"/>
                </xsl:variable>
                <xsl:if test="$parent.label != ''">
  ...
  <!-- yes: it's /db:section in the expression below -->
    ...
    <!-- if this is a nested section, label the parent -->
            <xsl:if test="../self::db:section     | ../self::db:sect1     | ../self::db:sect2     | ../self::db:sect3     | ../self::db:sect4     | ../self::db:sect5">
  ...
  <!-- what would the label be? -->
    ...
    <xsl:template match="db:qandadiv" mode="m:label-content">
        <xsl:if test="$autolabel.elements/db:qandadiv">
            <xsl:variable name="lparent" select="(ancestor::db:set        |ancestor::db:book        |ancestor::db:chapter        |ancestor::db:appendix        |ancestor::db:preface        |ancestor::db:section        |ancestor::db:simplesect        |ancestor::db:sect1        |ancestor::db:sect2        |ancestor::db:sect3        |ancestor::db:sect4        |ancestor::db:sect5        |ancestor::db:refsect1        |ancestor::db:refsect2        |ancestor::db:refsect3)[last()]"/>
        ...
        <xsl:variable name="prefix">
            <xsl:if test="$qanda.inherit.numeration != 0">
                ...
                <xsl:if test="ancestor::db:qandadiv">
                    <xsl:apply-templates select="ancestor::db:qandadiv[1]" mode="m:label-content"/>
                    <xsl:apply-templates select="ancestor::db:qandadiv[1]" mode="m:intralabel-punctuation"/>
                </xsl:if>
            </xsl:if>
        </xsl:variable>
        <xsl:variable name="inhlabel" select="ancestor-or-self::db:qandaset/@defaultlabel[1]"/>
    ...
    <xsl:template match="db:bibliography|db:glossary|db:index|db:setindex" mode="m:label-content">
        <xsl:if test="@label">
            <xsl:value-of select="@label"/>
        </xsl:if>
    </xsl:template>
    <xsl:template match="db:bibliodiv|db:glossdiv|db:indexdiv" mode="m:label-content">
        <xsl:if test="@label">
            <xsl:value-of select="@label"/>
        </xsl:if>
    </xsl:template>
  ...
  <!-- tables have to be handled specially because HTML tables with captions don't count -->
        <xsl:variable name="pchap" select="(ancestor::db:chapter                          |ancestor::db:appendix                          |ancestor::db:article[ancestor::db:book])[last()]"/>
        <xsl:choose>
            <xsl:when test="@label">
                <xsl:value-of select="@label"/>
            </xsl:when>
                ...
                <xsl:choose>
                    <xsl:when test="$pchap">
                        ...
                        <xsl:if test="$prefix != ''">
                            ...
                            <xsl:apply-templates select="$pchap" mode="m:intralabel-punctuation"/>
                        </xsl:if>
                        <xsl:variable name="count" as="xs:string*">
    ...
    <xsl:template match="db:figure|db:example|db:procedure" mode="m:label-content">
        <xsl:variable name="pchap" select="ancestor::db:chapter                         |ancestor::db:appendix                         |ancestor::db:article[ancestor::db:book]"/>
        <xsl:choose>
            <xsl:when test="@label">
                <xsl:value-of select="@label"/>
            </xsl:when>
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="ancestor::db:chapter or ancestor::db:appendix">
                        <xsl:number format="1" count="db:equation[db:title or db:info/db:title]" from="db:chapter|db:appendix" level="any"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:number format="1" count="db:equation[db:title or db:info/db:title]" level="any"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="db:orderedlist/db:listitem" mode="m:label-content">
      ...
      <!-- What!? This should never happen -->
        ...
        <xsl:number value="f:orderedlist-item-number(.)" format="{$type}"/>
    </xsl:template>
