...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:h="http://www.w3.org/1999/xhtml" xmlns:u="http://nwalsh.com/xsl/unittests#" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:fp="http://docbook.org/xslt/ns/extension/private" exclude-result-prefixes="db doc f fp ghost h m u xs html" version="2.0">
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:cals-phase-1">
...
each row and cell.</para>
    ...
    <xsl:template match="db:thead|db:tbody|db:tfoot" mode="m:cals-phase-1">
        <xsl:variable name="overhang" select="for $col in (1 to xs:integer(../@cols))    return 0"/>
        <xsl:copy>
            <xsl:copy-of select="@*"/>
...
-->
        <xsl:variable name="row">
            <xsl:copy>
                <xsl:copy-of select="@*"/>
                <xsl:apply-templates select="(db:entry|db:entrytbl)[1]" mode="m:cals-phase-1">
                    <xsl:with-param name="overhang" select="$overhang"/>
        ...
        <xsl:variable name="decremented-overhang" select="for $col in (1 to count($overhang))    return xs:integer(max(($overhang[$col] - 1, 0)))"/>
        <xsl:variable name="next-overhang" select="for $col in (1 to count($overhang))    return      xs:integer($decremented-overhang[$col]                  + $row/db:row/*[$col]/@ghost:morerows)"/>
        <xsl:apply-templates select="following-sibling::db:row[1]" mode="m:cals-phase-1">
    ...
    <xsl:template match="db:entry|db:entrytbl" mode="m:cals-phase-1">
        ...
        <xsl:variable name="container" select="(ancestor::db:tgroup                                           |ancestor::db:entrytbl)[last()]"/>
        <xsl:variable name="nextpos" select="f:skip-overhang($overhang, $prevpos+1)" as="xs:integer"/>
        <xsl:variable name="pos" as="xs:integer">
            <xsl:choose>
                <xsl:when test="@namest">
                    <xsl:variable name="name" select="@namest"/>
                    <xsl:variable name="colspec" select="$container/db:colspec[@colname=$name]"/>
                ...
                <xsl:when test="@spanname">
                    ...
                    <xsl:variable name="spanspec" select="$container/db:spanspec[@spanname=$name]"/>
...
-->
        ...
        <xsl:copy>
            ...
            <xsl:attribute name="ghost:morerows" select="if (@morerows)       then xs:integer(@morerows)       else 0"/>
            <xsl:call-template name="inherit-table-attributes">
        ...
        <xsl:for-each select="for $col in ($pos + 1 to $pos + $width - 1) return $col">
            <ghost:overlapped ghost:colnum="{.}">
                <xsl:attribute name="ghost:morerows" select="if ($entry/@morerows)         then xs:integer($entry/@morerows)         else 0"/>
            </ghost:overlapped>
        </xsl:for-each>
  ...
  -->
        <xsl:apply-templates select="(following-sibling::db:entry            |following-sibling::db:entrytbl)[1]" mode="m:cals-phase-1">
            <xsl:with-param name="overhang" select="$overhang"/>
...
-->
            <xsl:for-each select="for $col      in ($pos + $width to $container/@cols)      return $col">
                <xsl:variable name="col" select="."/>
                <xsl:choose>
                    <xsl:when test="$overhang[$col] = 0">
<!--
...
-->
    ...
    <xsl:template match="comment()|processing-instruction()|text()" mode="m:cals-phase-1">
        <xsl:copy/>
    </xsl:template>

<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="inherit-table-attributes">
...
be an integer and won't have a parent.</para>
  ...
  <!-- the table attributes come from:
       ...
       in that order -->
        ...
        <xsl:variable name="spanspec" select="$tgroup/db:spanspec[@spanname=$entry/@spanname]"/>
        <xsl:variable name="elements" as="element()*">
            <xsl:choose>
                <xsl:when test="$entry instance of element()">
                    <xsl:sequence select="($entry,           $row,           $spanspec,           $tgroup/db:colspec[@colname=$spanspec/@namest],           $tgroup/db:colspec[@colname=$entry/@namest],           f:find-colspec-by-colnum($tgroup, $colnum),           $tgroup,           $tgroup/parent::*)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:sequence select="($row,           f:find-colspec-by-colnum($tgroup, $colnum),           $tgroup,           $tgroup/parent::*)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="colspec" select="f:find-colspec-by-colnum($tgroup, $colnum)"/>
        <xsl:for-each select="('rowsep', 'colsep',                          'align', 'valign',     'char', 'charoff')">
            <xsl:variable name="attr" select="QName('', .)"/>
            <xsl:variable name="value">
                <xsl:choose>
                    <xsl:when test="f:find-element-by-attribute($elements, $attr)">
                        <xsl:value-of select="f:find-element-by-attribute($elements, $attr)     /@*[node-name(.) = $attr]"/>
                    </xsl:when>
                    <xsl:otherwise>
            ...
            <xsl:choose>
                <xsl:when test="f:find-element-by-attribute($elements, $attr)">
                    <xsl:attribute name="{string($attr)}" select="f:find-element-by-attribute($elements, $attr)           /@*[node-name(.) = $attr]"/>
                </xsl:when>
                <xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="adjust-column-widths">
...
on the <parameter>pixels.per.inch</parameter> parameter.</para>
    ...
    <xsl:template name="adjust-column-widths" as="element(html:colgroup)">
        ...
        <xsl:variable name="specify-widths" select="$default.table.column.widths != 0 or exists($colgroup/h:col/@width)"/>
            ...
            <xsl:for-each select="$colgroup/*">
                <xsl:choose>
                    ...
                    <xsl:when test="contains(@width, '*')">
                        <col ghost:rel="{substring-before(@width, '*')}" ghost:abs="{if (substring-after(@width, '*') = '')       then 0       else f:convert-length(substring-after(@width, '*'))}">
                            <xsl:copy-of select="@*"/>
                        </col>
                    </xsl:when>
                    <xsl:when test="matches(@width, '^\d+$')">
                        <col ghost:rel="0" ghost:abs="{f:convert-length(concat(@width,'px'))}">
                            <xsl:copy-of select="@*"/>
                        </col>
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <colgroup>
            <xsl:choose>
                                ...
                                <xsl:attribute name="width">
                                    <xsl:choose>
                                        <xsl:when test="$abspixels = 0">
                                            <xsl:value-of select="format-number(@ghost:abs div $pixels.per.inch,                                                         '0.00')"/>
                ...
                <xsl:otherwise>
                    <xsl:variable name="relTotal" select="sum($parsedcols/@ghost:rel)"/>
                    <xsl:variable name="pixelwidth" select="if (contains($table-width, '%'))          then f:convert-length($table.width.nominal)          else f:convert-length($table-width)"/>
                    <xsl:variable name="relwidth" select="$pixelwidth - sum($parsedcols/@ghost:abs)"/>
	    ...
	    <xsl:message>
	      ...
	      <xsl:value-of select="$relwidth"/>
            ...
            -->
                            <col class="tcol{position()}">
                                <xsl:copy-of select="@*"/>
                                <xsl:attribute name="ghost:rel" select="(@ghost:rel div $relTotal * $relwidth)          + @ghost:abs"/>
                            </col>
                        </xsl:for-each>
                    </xsl:variable>
                        ...
                        <xsl:when test="contains($table-width, '%')">
                            <xsl:for-each select="$convcols">
                                <col class="tcol{position()}">
                                    ...
                                    <xsl:if test="$specify-widths">
                                        <xsl:attribute name="width">
                                            <xsl:value-of select="format-number(@ghost:rel div $absTotal * 100,                                                         '0.00')"/>
                                            <xsl:text>%</xsl:text>
                                        </xsl:attribute>
                                    </xsl:if>
                                </col>
                                ...
                                <col class="tcol{position()}">
                                    ...
                                    <xsl:if test="$specify-widths">
                                        <xsl:attribute name="width">
                                            <xsl:choose>
                                                <xsl:when test="$abspixels = 0">
                                                    <xsl:value-of select="format-number(@ghost:rel div $pixels.per.inch,                          '0.00')"/>
                                                    <xsl:text>in</xsl:text>
                                                </xsl:when>
                                                <xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:convert-length">
    ...
    <xsl:function name="f:convert-length">
        ...
        <xsl:choose>
            ...
            <xsl:when test="matches($length, '^[\+\-]?\d+\.?\d*\w+$')">
                <xsl:variable name="spaced" select="replace($length,                '(^[\+\-]?\d+\.?\d*)(\w+)$',         '$1 $2')"/>
                <xsl:variable name="magnitude" select="xs:double(substring-before($spaced, ' '))"/>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:find-element-by-attribute">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:find-colspec-by-colnum">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:skip-overhang">
...
current column number.</para>
    ...
    <xsl:function name="f:skip-overhang">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="if ($overhang[$pos] = 0)        then $pos        else f:skip-overhang($overhang, $pos+1)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:colspec-colnum">
    ...
    <xsl:function name="f:colspec-colnum" as="xs:integer">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$colspec/preceding-sibling::db:colspec">
                <xsl:value-of select="f:colspec-colnum(           $colspec/preceding-sibling::db:colspec[1])                      + 1"/>
            </xsl:when>
            <xsl:otherwise>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:spanspec-colnum-start">
...
span.</para>
    ...
    <xsl:function name="f:spanspec-colnum-start" as="xs:integer">
        ...
        <xsl:value-of select="f:colspec-colnum($spanspec/ancestor::db:tgroup[1]          /db:colspec[@colname=$spanspec/@namest])"/>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:spanspec-colnum-end">
...
of a span.</para>
    ...
    <xsl:function name="f:spanspec-colnum-end" as="xs:integer">
        <xsl:param name="spanspec" as="element(db:spanspec)"/>
        <xsl:value-of select="f:colspec-colnum($spanspec/ancestor::db:tgroup[1]          /db:colspec[@colname=$spanspec/@nameend])"/>
    </xsl:function>

<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="generate-colgroup">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="generate-col">
