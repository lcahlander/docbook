...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:n="http://docbook.org/xslt/ns/normalize" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db doc f fn m n t xlink xs" version="2.0">
...
<!-- Create keys for quickly looking up olink targets -->
    ...
    <xsl:key name="targetptr-key" match="div|obj" use="concat(ancestor::document/@targetdoc, '/',                      @targetptr, '/', ancestor::document/@lang)"/>
...
<!-- Return filename of database -->
  ...
  <!-- This selection can be customized if needed -->
        <xsl:variable name="target.database.filename" select="$target.database.document"/>
      ...
      <!-- Get the xref text for this record -->
                ...
                <xsl:variable name="lang">
                    <xsl:variable name="candidate">
                        <xsl:for-each select="$target.database">
                            <xsl:value-of select="key('targetptr-key', $olink.key)/@lang"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <xsl:choose>
                     ...
                     falls back to no style -->
                        ...
                        <xsl:call-template name="substitute-markup">
                            ...
                            <xsl:with-param name="pagenumber">
                                <xsl:for-each select="$target.database">
                                    <xsl:value-of select="key('targetptr-key', $olink.key)/@page"/>
                                </xsl:for-each>
                            </xsl:with-param>
                            <xsl:with-param name="docname">
                                <xsl:for-each select="$target.database">
                                    <xsl:value-of select="key('targetdoc-key', $targetdoc)/div[1]/ttl"/>
                                </xsl:for-each>
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:when>
...
-->
  <!-- recursive template generates path to olink target directory -->
    ...
    <xsl:template name="t:olink-page-citation">
        ...
        <xsl:choose>
            ...
            <xsl:when test="not(starts-with(normalize-space($xrefstyle),                         'select:')                  and (contains($xrefstyle, 'page')                      or contains($xrefstyle, 'Page')))                 and $current.docid != ''                  and $current.docid != $targetdoc                 and olink.insert.page.number = 1 ">
  ...
  <!-- Don't add docname if pointing to root element -->
        <xsl:variable name="rootptr">
            <xsl:for-each select="$target.database">
                <xsl:value-of select="key('targetdoc-key', $targetdoc)/div[1]/@targetptr"/>
            </xsl:for-each>
        </xsl:variable>
