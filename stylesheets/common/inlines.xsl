...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="db f fn m t xlink xs" version="2.0">
...
<!-- ==================================================================== -->
    ...
    <xsl:template match="db:citebiblioid">
        <xsl:call-template name="t:inline-charseq">
            <xsl:with-param name="class" select="if (@class) then concat(local-name(.),'-',@class) else ()"/>
        </xsl:call-template>
    </xsl:template>
            ...
            <xsl:with-param name="content">
                <xsl:choose>
                    <xsl:when test=". castable as xs:dateTime">
                        <xsl:value-of select="format-dateTime(xs:dateTime(.),                     $dateTime-format,       f:l10n-language(.), (), ())"/>
                    </xsl:when>
                    ...
                    <xsl:when test=". castable as xs:gYearMonth">
                        <xsl:value-of select="format-date(xs:date(concat(.,'-01')),                 $gYearMonth-format,          f:l10n-language(.), (), ())"/>
                    </xsl:when>
                    <xsl:when test=". castable as xs:gYear">
                        <xsl:value-of select="format-date(xs:date(concat(.,'-01-01')),                 $gYear-format,          f:l10n-language(.), (), ())"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:apply-templates/>
                    </xsl:otherwise>
  ...
  <!-- todo: biblio-citation-check -->
    ...
    <xsl:template match="db:pob|db:street|db:city|db:state        |db:postcode|db:country|db:otheraddr">
        <xsl:call-template name="t:inline-charseq"/>
    </xsl:template>
...
<!-- in Addresses, for example -->
    <xsl:template match="db:honorific|db:firstname|db:givenname|db:surname        |db:lineage|db:othername">
        <xsl:call-template name="t:inline-charseq"/>
    </xsl:template>
</xsl:stylesheet>