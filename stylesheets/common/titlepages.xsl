...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:tmpl="http://docbook.org/xslt/titlepage-templates" xmlns:tp="http://docbook.org/xslt/ns/template/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:ghost="http://docbook.org/ns/docbook/ephemeral" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:fp="http://docbook.org/xslt/ns/extension/private" version="2.0" exclude-result-prefixes="db m t tp ghost xs f fp tmpl">
    <xsl:template name="t:titlepage">
        <xsl:param name="node" select="."/>
        <xsl:variable name="titlepage-content" select="(($node/db:title,$node/db:info/db:title,$ghost:title)[1],                          ($node/db:subtitle,$node/db:info/db:subtitle,$ghost:subtitle)[1],                          ($node/db:titleabbrev,$node/db:info/db:titleabbrev,$ghost:titleabbrev)[1],                          $node/db:info/*[not(self::db:title) and not(self::db:subtitle)                                          and not(self::db:titleabbrev)])"/>
...
<!-- ============================================================ -->
    ...
    <xsl:function name="fp:titlepage-templates" as="element(tmpl:templates)?">
        ...
        <xsl:variable name="templates" select="fp:titlepage-templates($node, $path,                         $titlepage-templates/tmpl:user-templates/*)"/>
        <xsl:choose>
            <xsl:when test="empty($templates)">
                <xsl:sequence select="fp:titlepage-templates($node, $path,                             $titlepage-templates/tmpl:system-templates/*)"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:function name="fp:titlepage-templates" as="element(tmpl:templates)?">
        ...
        <xsl:variable name="templates" select="($templates-list[fp:matching-template(.,$node,$pathstr)])[1]"/>
        <xsl:choose>
...
<!-- ============================================================ -->
    <xsl:template match="db:*" priority="100" mode="m:titlepage-template">
        ...
        <xsl:apply-templates select="$content[node-name(.) = node-name(current())]" mode="m:titlepage-content">
...
<!-- ============================================================ -->
    <xsl:template match="db:title[@ghost:generated='true']" mode="m:titlepage-content">
        ...
        <xsl:param name="mode" as="xs:string" required="yes"/>
        <xsl:if test="$context/self::db:bibliography                 or $context/self::db:preface                 or $context/self::db:dedication                 or $context/self::db:glossary                 or $context/self::db:setindex                 or $context/self::db:index                 or $context/self::db:abstract                 or $context/self::db:warning                 or $context/self::db:caution                 or $context/self::db:tip                 or $context/self::db:note                 or $context/self::db:important">
