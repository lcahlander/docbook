...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fp="http://docbook.org/xslt/ns/extension/private" exclude-result-prefixes="db doc f fp m mp xs" version="2.0">
    ...
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:preprocess">
...
oreder.</para>
    ...
    <xsl:function name="f:preprocess" as="document-node()">
      ...
      <!-- We must always perform this step in order to retain base URI -->
                ...
                <xsl:variable name="profile" select="if (index-of($steps, 'profile'))        then f:profile($preprofile, $profile.separator, $profile.lang,                                            $profile.revisionflag, $profile.role,                                            $profile.arch,                                            $profile.audience,                                            $profile.condition,                                            $profile.conformance,                                            $profile.os,                                            $profile.outputformat,                                            $profile.revision,                                            $profile.security,                                            $profile.userlevel,                                            $profile.vendor,                                            $profile.wordsize                                           )                             else $preprofile"/>
