...
<xsl:stylesheet xmlns="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:t="http://example.com/" xmlns:xlink="http://www.w3.org/1999/xlink" version="2.0">
    <xsl:namespace-alias stylesheet-prefix="t" result-prefix="xsl"/>
    <xsl:output method="xml" indent="no"/>
    <xsl:template match="/">
        <t:stylesheet xmlns:db="http://docbook.org/ns/docbook" exclude-result-prefixes="xs db" version="2.0">
                ...
                <t:choose>
                    ...
                    <t:otherwise>
                        <t:copy>
                            ...
                            <link xlink:type="arc" xlink:from="xlink-source-{generate-id(.)}" xlink:to="{{$targets[1]/@xlink:label}}"/>
                        </t:copy>
                    </t:otherwise>
                </t:choose>
            </t:template>
            ...
            <t:template match="*" mode="locators">
                ...
                <t:variable name="ends" as="element()*">
                    <t:for-each select="$extended/*[@xlink:type='arc' and @xlink:from = $from]">
                        ...
                        <t:sequence select="$extended/*[@xlink:type='locator'                                           and @xlink:href and @xlink:label = $to]"/>
                    </t:for-each>
                </t:variable>
                <t:choose>
                    <t:when test="$ends">
                    ...
                    <t:otherwise>
                        <t:for-each select="$extended/*[@xlink:type='arc' and @xlink:from = $from]">
                            <t:variable name="to" select="@xlink:to"/>
                            <t:if test="$extended/*[@xlink:type='resource'                                     and @xlink:label=$to and @xml:id]">
