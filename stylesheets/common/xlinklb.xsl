...
<xsl:stylesheet xmlns="http://docbook.org/ns/docbook" xmlns:db="http://docbook.org/ns/docbook" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xlink="http://www.w3.org/1999/xlink" version="2.0">
    <xsl:output method="xml" indent="no"/>
    <xsl:variable name="lb" select="'http://www.w3.org/1999/xlink/properties/linkbase'"/>
    <xsl:template match="db:info/db:extendedlink[db:link[@xlink:type='arc'                                                      and @xlink:arcrole=$lb]]">
        <xsl:apply-templates select="db:link[@xlink:type='arc' and @xlink:arcrole=$lb                                        and @xlink:actuate='onLoad'                                        and @xlink:from and @xlink:to]" mode="load-linkbase"/>
    </xsl:template>
