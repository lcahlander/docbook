...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="db doc f l m mp t xlink xs" version="2.0">
...
<!-- ==================================================================== -->
<!-- find title content -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="mp:title-content">
...
but for elements that have optional titles, it may be a computed string.
    ...
    <xsl:template match="db:bibliography|db:glossary|db:index|db:setindex                      |db:dedication|db:colophon" mode="mp:title-content" as="node()*">
        <xsl:param name="allow-anchors" select="false()" as="xs:boolean"/>
    ...
    <xsl:template match="db:tip|db:note|db:important|db:warning|db:caution" mode="mp:title-content" as="node()*">
        <xsl:param name="allow-anchors" select="false()" as="xs:boolean"/>
        <xsl:variable name="title" select="(db:title,db:info/db:title)[1]"/>
        <xsl:choose>
            <xsl:when test="$title">
    ...
    <xsl:template match="db:abstract" mode="mp:title-content" as="node()*">
        <xsl:param name="allow-anchors" select="false()" as="xs:boolean"/>
        <xsl:variable name="title" select="(db:title,db:info/db:title)[1]"/>
        <xsl:choose>
            <xsl:when test="$title">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:titleabbrev-content">
...
but for elements that have optional titles, it may be a computed string.
    ...
    <xsl:template match="*" mode="m:titleabbrev-content" as="node()*">
        <xsl:param name="allow-anchors" select="false()" as="xs:boolean"/>
        <xsl:variable name="titleabbrev" select="(db:titleabbrev,db:info/db:titleabbrev)[1]"/>
        <xsl:choose>
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:subtitle-content">
    ...
    <xsl:template match="*" mode="m:subtitle-content" as="node()*">
        <xsl:variable name="subtitle" select="(db:subtitle,db:info/db:subtitle)[1]"/>
        <xsl:choose>
            <xsl:when test="$subtitle">
                <xsl:apply-templates select="$subtitle" mode="m:subtitle-content"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:sequence select="()"/>
            </xsl:otherwise>
...
<!-- ============================================================ -->
  ...
  <!-- Switch to normal mode if no links -->
        <xsl:choose>
            <xsl:when test="descendant-or-self::db:footnote or                     descendant-or-self::db:anchor or                     descendant-or-self::db:link or                     descendant-or-self::db:olink or                     descendant-or-self::db:xref or                     descendant-or-self::db:indexterm or       (ancestor::db:title and @xml:id)">
                <xsl:apply-templates mode="mp:no-anchors"/>
            </xsl:when>
            <xsl:otherwise>
