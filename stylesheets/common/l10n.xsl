...
<xsl:stylesheet xmlns:mldb="http://docbook.github.com/ns/marklogic" xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xdmp="http://marklogic.com/xdmp" xmlns:l="http://docbook.sourceforge.net/xmlns/l10n/1.0" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" exclude-result-prefixes="doc f l t xs xdmp mldb" extension-element-prefixes="xdmp" version="2.0">
...
<!-- Pending resolution of bug 15636, this href has to be absolute. If you install
    ...
    <xdmp:import-module namespace="http://docbook.github.com/ns/marklogic" href="/DocBook/base/common/marklogic.xqy"/>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:user-localization-data">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:l10n-language">
...
target.</para>
    ...
    <xsl:function name="f:l10n-language" as="xs:string">
        ...
        <!-- can't do this one step: attributes are unordered! -->
                    <xsl:variable name="lang-scope" select="$target/ancestor-or-self::*                               [@lang or @xml:lang][last()]"/>
        ...
        <xsl:choose>
            <xsl:when test="$localization/l:l10n[@language=$adjusted.language]                     or f:check-locale($adjusted.language)">
                <xsl:value-of select="$adjusted.language"/>
            </xsl:when>
    <!-- try just the lang code without country -->
            <xsl:when test="$localization/l:l10n[@language=substring-before($adjusted.language,'_')]                     or f:check-locale(substring-before($adjusted.language,'_'))">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="lang-attribute">
    ...
    <xsl:template name="lang-attribute" as="attribute()?">
        ...
        <xsl:variable name="language">
            <xsl:choose>
                ...
                <xsl:otherwise>
        <!-- can't do this one step: attributes are unordered! -->
                    <xsl:variable name="lang-scope" select="$node/ancestor-or-self::*                               [@lang or @xml:lang][last()]"/>
                    <xsl:variable name="lang-attr" select="($lang-scope/@lang | $lang-scope/@xml:lang)[1]"/>
                    <xsl:choose>
                        <xsl:when test="string($lang-attr) = ''">
                            <xsl:value-of select="$l10n.gentext.default.language"/>
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:gentext">
...
context node.</para>
    ...
    <xsl:function name="f:gentext">
        ...
        <xsl:param name="key"/>
        <xsl:param name="lang"/>
        <xsl:variable name="l10n.gentext" select="($localization/key('l10n-gentext', concat($lang, '#', $key)),            f:get-locale($lang)/key('l10n-gentext', concat($lang, '#', $key)))[1]"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:value-of select="($localization/key('l10n-gentext', concat('en#', $key)),                              f:get-locale('en')/key('l10n-gentext', concat('en#', $key)))[1]/@text"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext">
        <refpurpose>Returns the generated text associated with a particular key
in a particular language (locale)</refpurpose>
        <refdescription>
            <para>This template finds the gentext associated with a specified key
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-template">
...
parameters.</para>
    ...
    <xsl:template name="gentext-template">
        ...
        <xsl:variable name="user.localization.nodes" select="$localization//l:l10n[@language=$lang]"/>
        <xsl:variable name="localization.nodes" select="f:get-locale($lang)"/>
        ...
        <xsl:variable name="user.context.nodes" select="$user.localization.nodes/key('l10n-context', concat($lang, '#', $context))"/>
        <xsl:variable name="context.nodes" select="$localization.nodes/key('l10n-context', concat($lang, '#', $context))"/>
        ...
        <xsl:variable name="template.node" select="(($user.context.nodes/l:template[@name=$name                                                    and @style                                                    and @style=$xrefstyle]                         |$user.context.nodes/l:template[@name=$name                                                    and not(@style)])[1],                          ($context.nodes/l:template[@name=$name                                                     and @style                                                     and @style=$xrefstyle]                          |$context.nodes/l:template[@name=$name                                                     and not(@style)])[1])[1]"/>
        <xsl:choose>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-template-exists">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-dingbat">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-startquote">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-endquote">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-nestedstartquote">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="gentext-nestedendquote">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:dingbat">
...
the English locale value will be used as the default.</para>
    ...
    <xsl:function name="f:dingbat" as="xs:string">
        ...
        <xsl:variable name="l10n.dingbat" select="($localization/key('l10n-dingbat', concat($lang, '#', $dingbat)),                          f:get-locale($lang)/key('l10n-dingbat', concat($lang, '#', $dingbat)))[1]"/>
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:value-of select="($localization/key('l10n-dingbat', concat('en#', $dingbat)),                              f:get-locale('en')/key('l10n-dingbat', concat('en#', $dingbat)))[1]"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:check-locale">
    ...
    <xsl:function name="f:check-locale" as="xs:boolean">
        ...
        <xsl:choose>
            <xsl:when test="function-available('mldb:check-locale')">
                <xsl:sequence use-when="function-available('mldb:check-locale')" select="mldb:check-locale($lang)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:sequence select="doc-available(concat($localepath, $lang,'.xml'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>

<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:get-locale">
...
<!-- ============================================================ -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:load-locale">
    ...
    <xsl:function name="f:load-locale" as="element(l:l10n)">
        ...
        <xsl:choose>
            <xsl:when test="function-available('mldb:load-locale')">
                <xsl:sequence use-when="function-available('mldb:load-locale')" select="mldb:load-locale($lang)"/>
            </xsl:when>
            <xsl:otherwise>
