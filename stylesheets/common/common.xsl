...
<xsl:stylesheet xmlns:f="http://docbook.org/xslt/ns/extension" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:db="http://docbook.org/ns/docbook" xmlns:mp="http://docbook.org/xslt/ns/mode/private" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:m="http://docbook.org/xslt/ns/mode" xmlns:ext="http://docbook.org/extensions/xslt20" xmlns:t="http://docbook.org/xslt/ns/template" xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns:fp="http://docbook.org/xslt/ns/extension/private" exclude-result-prefixes="db doc f fp m t xs ext" version="2.0">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="person-name">
...
and <tag>othercredit</tag>) or with the locale.</para>
    ...
    <xsl:template name="t:person-name">
        ...
        <xsl:variable name="personname" select="if ($node/self::db:personname)                         then $node                         else $node/db:personname"/>
        <xsl:variable name="style">
            <xsl:choose>
                <xsl:when test="$node/@role">
                    <xsl:value-of select="$node/@role"/>
                </xsl:when>
                <xsl:when test="($node/parent::db:author or $node/parent::db:editor                        or $node/parent::db:othercredit) and $node/parent::*/@role">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="person-name-family-given">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:to-uppercase">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="person-name-last-first">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="person-name-first-last">
...
template.</para>
    ...
    <xsl:template name="t:person-name-first-last">
        ...
        <xsl:if test="$node/db:surname">
            <xsl:if test="$node/db:honorific or $node/db:firstname or $node/db:givenname                   or ($node/db:othername and $author.othername.in.middle != 0)">
                <xsl:text> </xsl:text>
            </xsl:if>
            <xsl:apply-templates select="$node/db:surname[1]"/>
        </xsl:if>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="person-name-list">
...
<!-- ============================================================ -->
    <doc:mode xmlns="http://docbook.org/ns/docbook" name="m:number">
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:copyright-years">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:select-mediaobject">
...
to determine which object in the list is appropriate.</para>
    ...
    <xsl:template name="t:select-mediaobject">
        ...
        <xsl:variable name="mediaobject.index" select="f:select-mediaobject-index($olist)"/>
...
<!-- ====================================================================== -->
    <doc:function xmlns="http://docbook.org/ns/docbook" name="f:select-mediaobject-index">
...
or 0 if no object is selected.</para>
    ...
    <xsl:function name="fp:select-mediaobject-index" as="xs:integer">
        ...
        <xsl:choose>
          ...
          <!-- Phrase is used only for TeX Math when the output is FO -->
                        <xsl:when test="$object/self::db:textobject                           and $object/db:phrase                           and $object/@role='tex'                           and $stylesheet.result.type = 'fo'                           and $tex.math.in.alt != ''">
          ...
          <!-- The first textobject is a reasonable fallback -->
                        <xsl:when test="$object/self::db:textobject                           and $object[not(@role) or @role != 'tex']">
                            <xsl:value-of select="1"/>
                        </xsl:when>

          <!-- don't use graphic when output is FO, TeX Math is used
               and there is math in alt element -->
                        <xsl:when test="$object/ancestor::db:equation                           and $object/ancestor::db:equation/db:alt[@role='tex']                           and $stylesheet.result.type = 'fo'                           and $tex.math.in.alt != ''">
                            <xsl:value-of select="0"/>
                        </xsl:when>
          ...
          <!-- Otherwise, see if this one is a useable graphic -->
              ...
              <!-- peek inside imageobjectco to simplify the test -->
                                <xsl:when test="$object/self::db:imageobjectco">
                                    <xsl:value-of select="f:is-acceptable-mediaobject                                       ($object/db:imageobject)"/>
                                </xsl:when>
                                <xsl:otherwise>
    ...
    <doc:function name="f:is-acceptable-mediaobject">
...
object is recognized as a graphic.</para>
    ...
    <xsl:function name="f:is-acceptable-mediaobject" as="xs:integer">
        ...
        <xsl:variable name="filename" select="f:mediaobject-filename($object)"/>
        <xsl:variable name="ext" select="f:filename-extension($filename)"/>
        <xsl:variable name="data" select="$object/db:videodata                                     |$object/db:imagedata                                     |$object/db:audiodata"/>
        <xsl:variable name="explicit-format" select="lower-case($data/@format)"/>
        <xsl:variable xmlns:svg="http://www.w3.org/2000/svg" name="format" select="if ($explicit-format)                         then $explicit-format                         else                           if ($data/svg:*)                           then 'svg'                           else ''"/>
        <xsl:choose>
            <xsl:when test="$use.svg = 0 and $format = 'svg'">0</xsl:when>
            <xsl:when xmlns:svg="http://www.w3.org/2000/svg" test="$use.svg != 0 and $format = 'svg'">1</xsl:when>
    ...
    <xsl:function name="f:mediaobject-filename">
        <xsl:param name="object" as="element()"/>
        <xsl:variable name="data" select="$object/db:videodata                                     |$object/db:imagedata                                     |$object/db:audiodata                                     |$object"/>
    ...
    <xsl:function name="f:relative-uri" as="xs:string">
        ...
        <xsl:variable name="srcurl">
            <xsl:call-template name="t:strippath">
                <xsl:with-param name="filename" select="if (starts-with($absuri, 'file:/'))                               then substring-after($absuri, 'file:')                               else $absuri"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="srcurl.trimmed" select="f:trim-common-uri-paths($srcurl, $destdir)"/>
        <xsl:variable name="destdir.trimmed" select="f:trim-common-uri-paths($destdir, $srcurl)"/>
        <xsl:variable name="depth" select="count(tokenize($destdir.trimmed, '/')[. ne ''])"/>
...
<!-- ============================================================ -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="id">
...
<!-- ====================================================================== -->
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:check-id-unique">
...
the ID is not unique.</para>
    ...
    <doc:template xmlns="http://docbook.org/ns/docbook" name="t:check-idref-targets">
...
<varname>element-list</varname> and warns the user otherwise.</para>
    ...
    <xsl:template name="t:check-idref-targets">
        ...
        <xsl:if test="$linkend != ''">
            <xsl:variable name="targets" select="key('id',$linkend)"/>
            <xsl:if test="$targets                   and empty(index-of($element-list,node-name($targets[1])))">
                <xsl:message>
